...
int main (int argc, char **argv)
{
  ...
  while (1)
      ...
      if (sighup)
	{
	  ...
	  cache_reload(options, dnamebuff, domain_suffix);
	  if (resolv && (options & OPT_NO_POLL))
	    servers = last_server = 
	      check_servers(reload_servers(resolv->name, dnamebuff, servers), 
      ...
      if (!(options & OPT_NO_POLL))
	{
	  ...
	  if (latest && last_change > resolv_changed &&
	      (last_change < time(NULL) || resolv_changed == 0))
	    {
	      resolv_changed = last_change;
	      servers = last_server = 
		check_servers(reload_servers(latest->name, dnamebuff, servers),
			      interfaces, peerfd, peerfd6);
	    }
	}
