...
#define DUP_SIZE 1024 
struct file_info *dupl;
int dup_files = 0;

int swap, silent = TRUE;
int file_count = 0, sym_count = 0, dev_count = 0, dir_count = 0;
...
int exclude = 0;
struct exclude_info exclude_paths[EXCLUDE_SIZE];
...
unsigned int mangle(char *d, char *s, int size, int block_size, int uncompressed)
{
...
#ifdef __TI_SQUASHFS_LZMA
	if(!uncompressed && (res = compress_lzma_7z_buf(s, size, d, &c_byte)) == 0) {
	...
	if(uncompressed || c_byte >= (unsigned long)size) {
		memcpy(d, s, size);
...
void write_bytes(int fd, unsigned int byte, int bytes, char *buff)
{
	...
	if(off +(unsigned long) bytes > ((long long)1<<32) - 1 )
			    ...
			    squashfs_block start_block, unsigned int offset, unsigned short *block_list)
{
	...
	if(filename[0] == '\0') {
		...
		perror("Cannot stat file, skipping...");
	...
	{
		ERROR("Unknown inode type=%d", type );
		return SQUASHFS_INVALID;
	}
 ...
 */
unsigned short get_checksum(unsigned char *b, int l)
{
...
int duplicate(char *buffer, int bytes, unsigned short **block_list, int *start, int blocks)
{
	unsigned short checksum = get_checksum(buffer, bytes);
	int i;

		for(i = 0; i < dup_files; i ++)
			if(bytes == dupl[i].bytes && checksum == dupl[i].checksum) {
				unsigned char buffer1[SQUASHFS_FILE_MAX_SIZE];
				unsigned int dup_bytes = dupl[i].bytes, dup_start = dupl[i].start;
				while(dup_bytes) {
				...
				if(dup_bytes == 0) {
					TRACE("Found duplicate file, start 0x%x, size %d, checksum 0x%x\n", dupl[i].start,
						dupl[i].bytes, dupl[i].checksum);
					*block_list = dupl[i].block_list;
					*start = dupl[i].start;
					return 1;
				}
			}
		
	if(dup_files % DUP_SIZE == 0) {
		if((dupl = (struct file_info *) realloc(dupl, (dup_files + DUP_SIZE) * sizeof(struct file_info))) == NULL) {
			BAD_ERROR("Out of memory in dup_files table reallocation!\n");
		}
	}

	dupl[dup_files].bytes = bytes;
	dupl[dup_files].checksum = checksum;
	dupl[dup_files].start = *start;
	if((dupl[dup_files].block_list = (unsigned short *) malloc(blocks * sizeof(unsigned short))) == NULL) {
		BAD_ERROR("Out of memory allocating block_list\n");
	}
	
	memcpy(dupl[dup_files].block_list, *block_list, blocks * sizeof(unsigned short));
	dup_files ++;
	return 0;
}


squashfs_inode write_file(char *filename, unsigned int size, int *c_size)
{
	unsigned int file, start, file_bytes = 0, byte, block = 0;
	unsigned int c_byte;
	unsigned int read_size=(size > (SQUASHFS_MAX_FILE_SIZE)) ? (SQUASHFS_MAX_FILE_SIZE) : size;
	...
	if(size > read_size)
		ERROR("file %s truncated to %d bytes\n", filename, (int)(SQUASHFS_MAX_FILE_SIZE));
	...
	if((c_buffer = (char *) malloc((blocks + 1) << block_log)) == NULL) {
		BAD_ERROR("Out of memory allocating write_file buffer\n");
	}

	for(size = read_size; size; size -= byte) {
		byte = read(file, buff, size < block_size ? size : block_size);
		c_byte = mangle(c_buffer + file_bytes, buff, byte, block_size, noD);
		block_list[block ++] = c_byte;
		file_bytes += SQUASHFS_COMPRESSED_SIZE(c_byte);
	}
	
	close(file);
	
	start = bytes;
	if(duplicate(c_buffer, file_bytes, &block_listp, &start, blocks)) {
		*c_size = 0;
		goto wr_inode;
	}
	...
	while(encomp_entry < source) {
		...
		for(;;) {
			...
			if(n == old_root_entries) {
				for(n = 0; n < encomp_entry && strcmp(source_path[n], dir_name) != 0; n++);
				if(n == encomp_entry) {
					strcpy(source_path[encomp_entry], dir_name);
					encomp_entry++;
					return TRUE;
				}
			}
	...
	while(_readdir(linuxdir, filename, dir_name) != FALSE) {
		...
		switch(buf.st_mode & S_IFMT) {
			case S_IFREG: {
				...
				INFO("\t%.2f%% of uncompressed file size (%d bytes)\n", 
				     ((float) c_size / buf.st_size) * 100.0, (int)buf.st_size);
				break;
			}
			case S_IFDIR:
...
#define ADD_ENTRY(buf) \
	if(exclude == EXCLUDE_SIZE) {\
		ERROR("Exclude dir/file limit reached [%d entries]\n", EXCLUDE_SIZE);\
		return FALSE;\
	}\
...
int main(int argc, char *argv[])
{
	...
	for(; i < argc; i++) {
		if(strcmp(argv[i], "-b") == 0) {
			...
			printf("mksquashfs version 1.2, copyright (C) 2003 Phillip Lougher (phillip@lougher.demon.co.uk)\n");
...
printOptions:
	...
	if(delete) {
		...
		if((bytes = read_filesystem(root_name, fd, &sBlk, &inode_table, &inode_bytes, &data_cache,
				...
				&file_count, &sym_count, &dev_count, &dir_count, (squashfs_uid *) uids, &uid_count,
				(squashfs_uid *) guids, &guid_count,
				&total_bytes, &total_inode_bytes, &total_directory_bytes, add_old_root_entry)) == 0)
			BAD_ERROR("Failed to read existing filesystem - will not overwrite - ABORTING!\n");
