...
extern int duplicate(char *buffer, int bytes, unsigned short **block_list, int *start, int blocks);
extern void read_bytes(int fd, unsigned int byte, int bytes, char *buff);
...
int read_block(int fd, int *start, unsigned char *block, squashfs_super_block *sBlk)
{
	...
	if(SQUASHFS_COMPRESSED(c_byte)) {
		...
		read_bytes(fd, *start + offset, c_byte, (char *)buffer);

#ifdef __TI_SQUASHFS_LZMA
		if((res = decompress_lzma_7z_buf((char *)buffer, c_byte, block, &bytes)) == 0) {
			ERROR("zlib::compress failed, unknown error %d\n", res);
			return 0;
		}
#else
		...
		int *dev_count, int *dir_count)
{
	...
	for(cur_ptr = *inode_table; cur_ptr < *inode_table + bytes; files ++) {
		...
		TRACE("scan_inode_table: processing inode @ byte position 0x%x, type 0x%x\n", cur_ptr - *inode_table,
				inode.inode_type);
		switch((inode.inode_type - 1) % SQUASHFS_TYPES + 1) {
			case SQUASHFS_FILE_TYPE: {
				...
				if((c_buffer = malloc(inode.file_size)) == NULL)
					return FALSE;
				...
				*uncompressed_file += inode.file_size;
				...
				TRACE("scan_inode_table: reading file data, start 0x%x, size %d\n", start, file_bytes);
				read_bytes(fd, start, file_bytes, c_buffer);
	                        duplicate(c_buffer, file_bytes, &block_listp, &start, blocks);
...
#ifdef _THIS_WONT_WORK_UNLESS_READ_BYTES_RETURNS_SOMETHING_
	if(!read_bytes(fd, SQUASHFS_START, sizeof(squashfs_super_block), sBlk)) {
		ERROR("unable to read superblock on %s\n", source);
		goto failed_mount;
	}
#endif
		...
		void (push_directory_entry)(char *, squashfs_inode, int))
{
	...
	if((files = scan_inode_table(fd, start, end, root_inode_start, sBlk, &inode, &inode_table, &root_inode_block,
			uncompressed_file, uncompressed_directory, file_count, sym_count, dev_count, dir_count)) == 0) {
