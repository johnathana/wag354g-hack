...
extern char stbmatchtypes[MAXSTBTYPE][58];
...
void stbinfo_init()
{
	...
	if(!(fp = fopen("/proc/Cybertan/stb_info","w"))){
	  cprintf("error open stb_info file\n");
	  return 0;
	}
	fprintf(fp,"%s","clear");
	fclose(fp);
...
//by songtao 1012
struct stb_info *get_new_stbinfo(u_int8_t *haddr)
{
	int i;
	for(i=0; i<MAXSTBDEV; i++)
	{
		...
		if(oldstbinfo->xid !=0)
		{
		 cprintf("find existed xid!!!\n haddr=%0x:%0x:%0x:%0x:%0x:%0x\noldstbinfo->mac=%0x:%0x:%0x:%0x:%0x:%0x\n",haddr[0],haddr[1],haddr[2],haddr[3],haddr[4],haddr[5],
		oldstbinfo->mac[0],
		oldstbinfo->mac[1],
		oldstbinfo->mac[2],
		oldstbinfo->mac[3],
		...
		oldstbinfo->mac[5]
		 ...
		 if(
		  (oldstbinfo->mac[0]==haddr[0])
		  &&(oldstbinfo->mac[1]==haddr[1])
		  &&(oldstbinfo->mac[2]==haddr[2])
		  &&(oldstbinfo->mac[3]==haddr[3])
		  &&(oldstbinfo->mac[4]==haddr[4])
		  &&(oldstbinfo->mac[5]==haddr[5])
		  ){
		         cprintf("find the existed stb dev!!!\n");
			 oldstbinfo->existed = 1;
			 return oldstbinfo;
		    }
		}
	}
...
// by songtao 1011
//struct stb_info *lookup_stbinfo(unsigned int id,u_int32_t ipaddr,u_int8_t *haddr)
{
	int i;
	for(i=0; i<MAXSTBDEV; i++)
	{
		...
		//	by songtao 1011
/*		if((oldstbinfo->xid == id)||((oldstbinfo->ipaddr == ipaddr)
		  &&((oldstbinfo->mac[0]==haddr[0])
	           &&(oldstbinfo->mac[1]==haddr[1])
		   &&(oldstbinfo->mac[2]==haddr[2])
		   &&(oldstbinfo->mac[3]==haddr[3])
