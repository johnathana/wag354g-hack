...
extern int read_macip(char *line, char *var);
extern  int read_hostnameip(char *line, char *value);
...
static struct config_keyword keywords[] = {
	...
	//wwzh
	{"macip", 	read_macip, NULL, ""},
	{"hostip",  	read_hostnameip, NULL, ""},
	//leijun
	{"share_wanip", read_mac_index, NULL, ""},
	{"share_wanip_comm", read_u32, &(server_config.share_wanip_comm), "0"},
	{"single_pc_mode", read_u32, &(server_config.single_pc_mode),	"0"},

#ifdef STB_SUPPORT
        {"stb_support", read_u32,  &(server_config.stb_support),        "0"},
        {"stb_server",  read_ip,  &(server_config.stb_server),  "0.0.0.0"},
#endif
...
int read_config(char *file)
{
  ...
  memset((void *)&share_wan_ip, 0, sizeof(share_wan_ip));      
	...
	while (fgets(buffer, 80, in)) {
		...
		*line = '\0';
		...
		for (i = 0; strlen(keywords[i].keyword); i++)
			...
			{
//				printf("token = %s, line = %s, var = %s\n", token, line, keywords[i].var);
...
void read_leases(char *file)
{
	...
	while (i < server_config.max_leases && (fread(&lease, sizeof lease, 1, fp) == 1)) {
		...
		if (ntohl(lease.yiaddr) >= ntohl(server_config.start) && ntohl(lease.yiaddr) <= ntohl(server_config.end)) {	// Fix by honor
			DEBUG(LOG_DEBUG, "Reload %x", ntohl(lease.yiaddr));
			lease.expires = ntohl(lease.expires);
			if (!server_config.remaining) lease.expires -= get_time(0);
