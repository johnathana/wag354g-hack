...
rip_interface_new ()
{
 ...
 //modified by zls 2005-0106
  ri->broadcast_default = 0;
  ri->ri_broadcast=ri->broadcast_default;
...
//add by leijun 2004-1231
DEFUN (rip_broadcast_mode,
       rip_broadcast_mode_cmd,
       "broadcast <0-1>",
       "Set broadcase mode\n"
       "broadcast\n")
{       //modified by zls 2005-0106
        ...
        ifp=(struct interface *)vty->index;
        ri=ifp->info;
        ri->ri_broadcast = atoi(argv[0]);
        /*if (rip)
        {
                rip->broadcast_flag = atoi (argv[0]);
                printf("broadcast = %d\n", rip->broadcast_flag);
        }*/
        return CMD_SUCCESS;
}
...
rip_if_init ()
{
  ...
  install_element (INTERFACE_NODE, &rip_broadcast_mode_cmd);//modified by zls 2005-0115
}
