...
#ifdef ATMARP_DEBUG
        syslog(LOG_CRIT, "libatm.arpd.send_packet packet len %d, header >>>", length );
        syslog(LOG_CRIT, "{   hrd:  0x%04X", ntohs(((unsigned short *) data)[4]));
        syslog(LOG_CRIT, "    pro:  0x%04X", ntohs(((unsigned short *) data)[5]));
        syslog(LOG_CRIT, "    shtl: 0x%02X", ((unsigned char  *) data)[12] );
        syslog(LOG_CRIT, "    sstl: 0x%02X", ((unsigned char  *) data)[13] );
        syslog(LOG_CRIT, "    op:   0x%04X", ntohs(((unsigned short *) data)[7]));
        syslog(LOG_CRIT, "    spln: %d    }",
...
#ifdef ATMARP_DEBUG
    syslog(LOG_CRIT, "libatm.arpd.send_packet.bad write");
#endif
