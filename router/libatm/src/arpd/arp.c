...
#define ENV_FILE_LEN   4096	/* Length of env file /proc/ticfg/env */
#define ENV_FILE_NAME   "/proc/ticfg/env"	/* Path and name of env file */
...
static void arp_read_env( char *var, char *env, int  envlen) ;
...
put_addr(
  ...
  unsigned char *num_tl,
  unsigned char *sub_tl)
{
    ...
    if (!*addr->sas_addr.pub) *sub_tl = 0;
        ...
        /*
         * syslog( LOG_CRIT, "putting E164 address len %d", strlen(addr->sas_addr.pub));
         */
	*num_tl = strlen(addr->sas_addr.pub) | TL_E164;
    ...
    if (!*addr->sas_addr.prv) *num_tl = 0;
        ...
        /*
         * syslog( LOG_CRIT, "putting EISA address");
         */
	*num_tl = ATM_ESA_LEN;
...
/*
 * Function: send_arp
 * Description: Sends arp for ATPARP. Checks env variable ATM_ARP_HWA to get the E164 (20B)
 * HWA address to send as source hardware address.
 */
static void 
send_arp(
  int fd,
  ...
  struct sockaddr_atmsvc *remote_addr
    ...
    unsigned char *here;
    char  aenv[ATM_E164_LEN+1] ;

/*
 *   syslog( LOG_CRIT, "Func: send_arp [op: %d]", op );
 */

    /*
     * If the local address is null, check environment variable
     * ATM_ARP_HWA first ...
     */
    if (!local_addr) {
       memset(&local,0,sizeof(struct sockaddr_atmsvc));
       arp_read_env("ATM_ARP_HWA", aenv, ATM_E164_LEN ) ;
    
       if(aenv[0]) {
          local.sas_family = AF_ATMSVC ;
          strcpy( local.sas_addr.pub, aenv ) ;
       } 
       else {
    ...
    syslog( LOG_CRIT, "send_arp: op %d, pub E164 addr [%s]",  
                          op, local_addr->sas_addr.pub );
...
/* 
 * Function: arp_read_env
 * Description:
 * This function takes the name of the variable, any array to put 
 * the variable into, and the length of the array, as the input.
 *
 * If the variable is not there or there is no value, this function
 * will set char '\0' in the first char of the array.
 */
...
arp_read_env(
   ...
   /* Variable is null to begin with and will stay null if */
   /* not found in the file                                */
   env[0] = '\0' ;

   /* Allocate space for reading the variables             */
   buf = malloc( ENV_FILE_LEN ) ;

   /* Return with null variable value if no memory         */
   if( !buf ) 
      return ;

   /* Open the file that hold all the environment variables*/
   fd = open(ENV_FILE_NAME, O_RDONLY);

   if(fd > 0) {

      /* Read upto buffer length the variables in the buf  */
      /* that is only how much we will search              */
      res = read(fd, buf, ENV_FILE_LEN );

      if(res > 0) {
	 buf[res] = 0;
	 ptr = buf;
       
         /* Each name value pair is in one line separated  */
         /* by the tab char. Go through each, compare with */
         /* the input parameter and if found, copy the     */
         /* value in the second input parameter            */
	 while((pos = strchr(ptr, '\n')) )
	 {
		*pos = 0;
		...
		if(pos1)
		{
			*pos1 = 0;
			pos1++;
                        /* Make sure that the variable value is     */ 
                        /* not greater than we can hold             */
			if(!strcmp(ptr, var)) {
                                if( strlen(pos1) > envlen )
                                   pos1[envlen] = '\0' ;
			        strcpy(env, pos1) ;
                                /*
                                 *syslog(LOG_CRIT, "EnvRead: %s", env ) ;
                                 */
                                break ;
                        }
		}
      ...
      /* Can't open the file that hold all the environment variables*/
      syslog( LOG_CRIT, "file opening error" ) ;
      free( buf ) ;
      return ;
   }

   /* Now, we are done with memory and file             */
   close(fd);
   free( buf ) ;
}
...
static void inarp_request(ENTRY *entry)
{
    ...
    syslog(LOG_CRIT,"sending an InARP request for each IP interface");
  ...
  const struct atm_qos *qos,int sndbuf,int flags)
{
    ...
    Q_INSERT_HEAD(entry->vccs,vcc);
    Q_INSERT_HEAD(itf->table,entry);
    syslog(LOG_CRIT,"atm arp: ioctl_set_pvc done");
    return 0;
}
