...
{	
	...
	{		
		...
		alert('VCI-' + errmsg.err126 + '[32 - 65535].');
...
function valid_vci(I)
{
	...
	if ( !(d>=32 && d<=65535) )
	{
		//alert('VCI value is out of range [32 - 65535]');
		alert('VCI-' + errmsg.err126 + '[32 - 65535].');
...
//		alert(M1 +' value is out of range ['+ start + ' - ' + end +']');
		if (( M == "Routeranzahl") || ( M == "Fragmentierungsschwellen" ) || ( M == "RTS-Schwelle"))
			alert(M1 + errmsg.err126a + ' [' + start + ' - ' + end + '].');
		else
			alert(M1 + '-' + errmsg.err126  + ' [' + start + ' - ' + end + '].');
		...
		for (var i=0; i<pairs.length; i++) {
			...
			for(iln = 0; iln < len; iln++) {
				...
				if (ch >= '0' && ch <= '9' || ch >= 'a' && ch <= 'f') {
					...
					alert(errmsg.err135 + nameVal + errmsg.err136 + oldmac);
...
function valid_ip(F,N,M1,flag){
	...
	if(m[0] == 127 || m[0] == 224){
		...
		alert(M + errmsg.err143);
		for(i=0;i<4;i++)
			eval(N+"_"+i).value = eval(N+"_"+i).defaultValue;
...
function valid_ip_gw(F,I,N,G)
{
	...
	if(!(NM[0] == "255" && NM[1] == "255" && NM[2] == "255" && NM[3] == "255"))
	{
	for(i=0;i<4;i++){
