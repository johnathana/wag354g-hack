 ...
 * SPECIFICALLY DISCLAIMS ANY IMPLIED WARRANTIES OF MERCHANTABILITY, FITNESS
 * FOR A SPECIFIC PURPOSE OR NONINFRINGEMENT CONCERNING THIS SOFTWARE.
 * $Id: wireless.c,v 1.1.1.2 2005/03/28 06:58:29 sparq Exp $
 */
...
int wireless_get_active_mac()
{
                                                                                	...
                                                                                	for (i = 0; i < MAX_STATIONS; i++)
	{
		...
		if (!strcmp(tmp, ""))
		 ...
		 sprintf(macaddr, "%02x:%02x:%02x:%02x:%02x:%02x", (station_table[i].macAddr[0] & 0xff), (station_table[i].macAddr[1] & 0xff), (station_table[i].macAddr[2] & 0xff), (station_table[i].macAddr[3] & 0xff), (station_table[i].macAddr[4] & 0xff), (station_table[i].macAddr[5] & 0xff));                                                        	
