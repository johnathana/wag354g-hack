...
#endif
int ej_vpn_logmsg(int eid,webs_t wp,int argc, char_t **argv)
{    
    	char *arg;
        char *temp="0";
        int count;
       ...
       if(gozila_action )
        {
          ...
          selet_flags=2;//atoi(temp);--zhangbin
	  printf("sel_sysLogType= %s\n", temp);
          if(((count == 0) && (!strcmp(temp, "0"))) || ((count == 1) && (!strcmp(temp, "1")))||((count==2)&&(!strcmp(temp,"2")))||((count==3)&&(!strcmp(temp,"3")))||((count==4)&&(!strcmp(temp,"4"))))
...
int
ej_dump_vpnlog(int eid,webs_t wp,int argc,char_t **argv)
{
	FILE *fp;
	char line[254];
	int i = 0;
...
////////////////////selet display type => system ,vpn ,access,..//////////////
  ...
  if((old_selet_type!=selet_type)||(refresh==2)){
	  ...
	  old_selet_type=selet_type;
          show_page_head= create_vpnpage(head,old_selet_type,selet_yes);
  }
...
struct show_page* create_vpnpage(struct logmsg *head_node,int type,int yes)
{
         struct logmsg *ptr;
