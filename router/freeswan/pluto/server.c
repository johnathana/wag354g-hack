 ...
 * for more details.
 *
 * RCSID $Id: server.c,v 1.1.1.3 2005/08/25 06:27:25 sparq Exp $
 */
...
//add by zhs for SIGSEGV ,2005-4-13
//static svolatile sig_atomic_t sigsegvflag =FALSE;
static void segvhandler(int sig )
{
//	sigsegvflag =TRUE;
	char buf[30];
	memset(buf,0,sizeof(buf));
	sprintf(buf,"rev sig SIGSEGV (%d) and exit pluto\n",sig);
	send_log(buf);
	exit_pluto(0);
}
//endif
