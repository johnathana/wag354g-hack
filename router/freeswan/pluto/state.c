 ...
 * for more details.
 *
 * RCSID $Id: state.c,v 1.1.1.3 2005/08/25 06:27:26 sparq Exp $
 */
...
find_sender(size_t packet_len, u_char *packet)
{
 ...
 /*add by zhs ,Dec 24,2004
  *find_ph2_st_by_ph1:
  */
struct state * find_ph2_st_by_ph1_st(struct state * p1st, u_int8_t protoid)
{
   struct state *st;
   int i;
   struct host_pair *p1host = p1st->st_connection->host_pair;
   struct connection *p1_c = p1st->st_connection;
 //     if (bogus) *bogus = FALSE;
   for (i = 0; i < STATE_TABLE_SIZE; i++)
          for (st = statetable[i]; st != NULL; st = st->st_hashchain_next)
              if ( (IS_IPSEC_SA_ESTABLISHED(st->st_state))
                  && (p1host == st->st_connection->host_pair)
                  && (same_peer_ids(p1_c, st->st_connection, NULL)))
                  {
...
/****************************************************
 *delete_old_states:delete the old states that have
 *                 the same connection.
 *add by zhs for isakmp_sa_timeout_bug,Feb 2,2005
 */
void delete_old_states(const struct state *newest_st)
{
    struct state *st;
    int i;
    struct host_pair *newest_host = newest_st->st_connection->host_pair;
    struct connection *newest_c = newest_st->st_connection;
    so_serial_t newest_serno = newest_st->st_serialno;
                                                                                                                           
                                                                                                                           
    for (i = 0; i < STATE_TABLE_SIZE; i++)
        for (st = statetable[i]; st != NULL; st = st->st_hashchain_next)
            if ( (IS_IPSEC_SA_ESTABLISHED(st->st_state))
                && (newest_host == st->st_connection->host_pair)
                && (same_peer_ids(newest_c, st->st_connection, NULL))
                && (newest_serno > st->st_serialno) )
                {
                    delete_state(st);
                }
}
...
//zhangbin
		...
		sprintf(status_file,"/tmp/%s_st",c->name);
		if((state_fd = fopen(status_file,"w")) == NULL)
		{
			log("cannot open ipsec_status!\n");
		}
		fprintf(state_fd,"1");
		fclose(state_fd);		
#endif
