 ...
 * for more details.
 *
 * RCSID $Id: demux.c,v 1.1.1.3 2005/08/25 06:27:25 sparq Exp $
 */
...
//zhangbin
		    {
			    ...
			    sprintf(err_msg,"2rIKE[\"%s\"] ERROR: %s :%s",sender->st_connection->name,fromstr,strerror(ee->ee_errno));
			    send_log(err_msg);
		    }		    
		    cur_state = old_state;
		}
...
#endif
{
	...
	if ((unsigned long) st->st_tpacket.len >
	    ...
	    send_log("send_packet(): really too big!");
	    return FALSE;
	}
	ptr = ike_pkt;
	/** Add Non-ESP marker **/
	...
	//zhangbin
	{
		...
		sprintf(buf_e,"sendto on %s to %s:%u failed in %s", c->interface->rname, ip_str(&c->that.host_addr), (unsigned)c->that.host_port, where);
		send_log(buf_e);
	}
	return FALSE;
    }
...
//zhangbin for blockip --2004/04/26
struct blockip blockip[BLOCKIP_NUM];
...
#define SEND_NOTIFICATION(t) { \
    ...
    sprintf(buf,"%s %s,line%d send_notification\n",__FILE__,__FUNCTION__,__LINE__);\
    send_log(buf); \
...
#define SEND_NOTIFICATION(t) { \
    ...
    //zhangbin for blockip
    for(;block_j < BLOCKIP_NUM;block_j++)
   {
		//log("block_j=%d:name=%s,timeout=%d,max_timeout=%d,now=%d,cnt=%d,max_cnt=%d,ip=%08x",block_j,blockip[block_j].name,blockip[block_j].timeout,blockip[block_j].max_timeout,now(),blockip[block_j].cnt,blockip[block_j].max_cnt,blockip[block_j].ip);
		//log("s_addr=%08x",md->sender.u.v4.sin_addr.s_addr);
	   	 if(md->sender.u.v4.sin_addr.s_addr == blockip[block_j].ip && (now() - blockip[block_j].timeout < blockip[block_j].max_timeout) && blockip[block_j].cnt >= blockip[block_j].max_cnt)
		{
			log("will send blockip!!");
			//send_notification(st,80,NULL,0,NULL,NULL,blockip[j].name,blockip[j].max_timeout,PROTO_ISAKMP);
			send_blockip(md,80,blockip[block_j].name,blockip[block_j].max_timeout);
			return;
		}
    }
     ...
     */
    passert(STATE_IKE_FLOOR <= from_state && from_state < STATE_IKE_ROOF);//zhs change "<=" to "<"
    smc = ike_microcode_index[from_state - STATE_IKE_FLOOR];

#ifdef ZHS_DEBUG
    smc_index=from_state - STATE_IKE_FLOOR;
#endif
    if (st != NULL)
    {
...
//zhangbin		    
		    {
			    char p_msg[200];//zhs change 100 to 200
			    memset(p_msg,0,sizeof(p_msg));//add by zhs
			    sprintf(p_msg,"%s message ignored:payload type (%s)", excuse, enum_show(&payload_names, np));
			    send_log(p_msg);
			}
    ...
    //zhangbin
		    ...
		    { 
			char pl_msg[100];
    			sprintf(pl_msg,"%s message ignored: unexpected payload type (%s).", excuse, enum_show(&payload_names, np));
			    send_log(pl_msg);
			}
		...
		//zhangbin
		{
			char f_msg[180];//zhs change 80 to 180
			memset(f_msg,0,sizeof(f_msg));//add by zhs
			sprintf(f_msg,"%s malformed payload in packet", excuse);
			send_log(f_msg);
		}
		SEND_NOTIFICATION(PAYLOAD_MALFORMED);
		return;
	...
	if (md->hdr.isa_np != ISAKMP_NEXT_HASH)
	{
	    ...
	    {
		    char mal_msg[80];
		    sprintf(mal_msg,"%s","malformed Quick Mode message: does not start with a HASH payload");
		    send_log(mal_msg);
	   }
#endif	    
	    SEND_NOTIFICATION(PAYLOAD_MALFORMED);
    ...
    /* Handle (or ignore) Delete/Notification/VendorID Payloads */
	...
	for (p = md->chain[ISAKMP_NEXT_N]; p != NULL; p = p->next)
	{
		...
		if(NULL!=st)//add by zhs for pluto_exit exception bug,2005-4-21
		{
			char nt_msg[100],nt_type[80];
			memset(nt_msg,0,sizeof(nt_msg));
			memset(nt_type,0,sizeof(nt_type));
			sprintf(nt_type,"2nIKE[\"%s\"] Rx {{ Notify : %s",st->st_connection->name,enum_show(&ipsec_notification_names, p->payload.notification.isan_type));
			switch(p->payload.notification.isan_type)
			{
				case INVALID_PAYLOAD_TYPE:
					sprintf(nt_msg,"2rIKE[\"%s\"] **Check your ISAKMP Pre-share Key setting !",st->st_connection->name);
					break;
				case PAYLOAD_MALFORMED:
					sprintf(nt_msg,"2rIKE[\"%s\"]] **Check your PFS setting !",st->st_connection->name);
					break;
				case NO_PROPOSAL_CHOSEN:
					sprintf(nt_msg,"2rIKE[\"%s\"] **Check your Encryption,Authentication method and PFS settings !",st->st_connection->name);
					break;
				case INVALID_ID_INFORMATION:
					sprintf(nt_msg,"2rIKE[\"%s\"] **Check your Local/Remote Secure Group settings !",st->st_connection->name);
					break;
				default:
					break;
			}
...
#ifdef ZHS_DEBUG
{//add by zhs only for debug
	char debug[100];
	//int offset =(char *)smc-(char *)ike_microcode_index;
        
	//smc_index=offset/(sizeof(struct state_microcode));
	memset(debug,0,sizeof(debug));
        sprintf(debug,"%s,smc index is (%d)%d\n",__FUNCTION__,smc_index,from_state - STATE_IKE_FLOOR);
	send_log(debug); 
...
//zhangbin for vpn-log
		{
      			if(IS_IPSEC_SA_ESTABLISHED(st->st_state))
      			  {
				char vpnmsg[100],suc_msg[100],enc_str[8],auth_str[8];
				memset(vpnmsg,0,sizeof(vpnmsg));
				memset(suc_msg,0,sizeof(suc_msg));
				memset(enc_str,0,sizeof(enc_str));
				memset(auth_str,0,sizeof(auth_str));

				send_log("for ipsec-log!");
				switch(st->st_esp.attrs.transid)
				{
					case ESP_NULL:
						sprintf(enc_str,"%s","NULL");
					       break;
				       	case ESP_DES:
				       	       sprintf(enc_str,"%s","DES");
					       break;
					case ESP_3DES:
			       		       sprintf(enc_str,"%s","3DES");
					       break;
					default:
					       break;
				}
				switch(st->st_esp.attrs.auth)
				{
					case AUTH_ALGORITHM_NONE:
						sprintf(auth_str,"%s","NONE");
					       break;
				        case AUTH_ALGORITHM_HMAC_MD5:
				       	       sprintf(auth_str,"%s","MD5");
					       break;
					case AUTH_ALGORITHM_HMAC_SHA1:
				       	       sprintf(auth_str,"%s","SHA1");
					       break;
					default:
					       break;
				}
						       
				sprintf(vpnmsg,"IKE[\"%s\"] ESP_SA %s / %s / %d sec / SPI=[%08x:%08x]",st->st_connection->name,enc_str,auth_str,st->st_esp.attrs.life_seconds,ntohl(st->st_esp.our_spi),ntohl(st->st_esp.attrs.spi));
				sprintf(suc_msg,"<FONT color=blue>IKE[\"%s\"] Set up ESP tunnel with %s Success!</FONT>",st->st_connection->name,ip_str(&st->st_connection->that.host_addr));
				log("vpnmsg=%s,suc_msg=%s!",vpnmsg,suc_msg);
			    	send_log(vpnmsg);
			    	send_log(suc_msg);
			  }
			else if(IS_ISAKMP_SA_ESTABLISHED(st->st_state))
	                {
				char msg1[100],msg2[100],oakley_enc[8],oakley_auth[8],group[15],temp1[3],temp2[3],temp3[3],temp4[3],i_ck1[10],i_ck2[10],r_ck1[10],r_ck2[10];
				int i = 0,j = 0;
				log("in isakmp!");
				...
				memset(oakley_enc,0,sizeof(oakley_enc));
				memset(oakley_auth,0,sizeof(oakley_auth));
				...
				switch(st->st_oakley.encrypt)
				{
					case OAKLEY_3DES_CBC:
						sprintf(oakley_enc,"%s","3DES");
					       break;
			       		case OAKLEY_DES_CBC:
				       	       sprintf(oakley_enc,"%s","DES");
					       break;
					default:
					       break;
				}
				...
				{
					case OAKLEY_MD5:
						sprintf(oakley_auth,"%s","MD5");
					       break;
				       	case OAKLEY_SHA:
				       	       sprintf(oakley_enc,"%s","SHA");
					       break;
					default:
					       break;
				}
				switch(st->st_oakley.group->group)
				{
					case OAKLEY_GROUP_MODP768:
						sprintf(group,"%s","MODP_768");
					       break;
				       	case OAKLEY_GROUP_MODP1024:
				       	       sprintf(group,"%s","MODP_1024");
					       break;
					default:
					       break;
				}

				sprintf(msg1,"2nIKE[\"%s\"] ISAKMP SA CKI=[%s %s] CKR=[%s %s]",st->st_connection->name,i_ck1,i_ck2,r_ck1,r_ck2);
				sprintf(msg2,"2nIKE[\"%s\"] ISAKMP SA %s / %s /PreShared / %s / %d sec(*%d sec)",st->st_connection->name,oakley_enc,oakley_auth,group,st->st_oakley.life_seconds,st->st_oakley.life_seconds);
				send_log(msg1);
				send_log(msg2);
		    	}
		}//end for vpn-log			
		...
		|| IS_IPSEC_SA_ESTABLISHED(st->st_state))
		{
			...
			{
     	 			//if(IS_IPSEC_SA_ESTABLISHED(st->st_state))
     	 			if((st->st_state == STATE_QUICK_I2) || (st->st_state == STATE_QUICK_R2))
      				{
				char enc_str[8],auth_str[8];
				memset(enc_str,0,sizeof(enc_str));
				memset(auth_str,0,sizeof(auth_str));
				...
				sprintf(msg1,"2nIKE[\"%s\"] ESP_SA %s / %s / %ld sec / SPI=[%08x:%08x]",st->st_connection->name,enc_str,auth_str,st->st_esp.attrs.life_seconds,ntohl(st->st_esp.our_spi),ntohl(st->st_esp.attrs.spi));//zhs modified for warning msg
                    		send_log(msg1);
				sprintf(msg2,"2bIKE[\"%s\"] Set up ESP tunnel with %s Success!",st->st_connection->name,ip_str(&st->st_connection->that.host_addr));
                    		send_log(msg2);
				 {//add by zhs to delete old ipsec state,Feb 2,2005
                                    delete_old_states(st);
                                   // show_states_status();//add by zhs only for show state
                                }//end by zhs

			  }
			else if(IS_ISAKMP_SA_ESTABLISHED(st->st_state))
	                {
				char oakley_enc[8],oakley_auth[8],group[15],temp1[3],temp2[3],temp3[3],temp4[3],i_ck1[10],i_ck2[10],r_ck1[10],r_ck2[10];
				int i = 0,j = 0;
				memset(temp1,0,sizeof(temp1));
				memset(temp2,0,sizeof(temp2));
				memset(temp3,0,sizeof(temp3));
				...
				sprintf(msg1,"2nIKE[\"%s\"] ISAKMP SA CKI=[%s %s] CKR=[%s %s]",st->st_connection->name,i_ck1,i_ck2,r_ck1,r_ck2);
     		               send_log(msg1);
				sprintf(msg2,"2nIKE[\"%s\"] ISAKMP SA %s / %s /PreShared / %s / %ld sec(*%ld sec)",st->st_connection->name,oakley_enc,oakley_auth,group,st->st_oakley.life_seconds,st->st_oakley.life_seconds);//zhs modified for warning msg
                		send_log(msg2);
		    	}
		}//end for vpn-log			
...
#if 0	    
//zhangbin:hope it can work
	    if(IS_IPSEC_SA_ESTABLISHED(st->st_state))
      	  {
				char ipsec_msg1[100],ipsec_msg2[100],enc_str[8],auth_str[8];
				memset(ipsec_msg1,0,sizeof(ipsec_msg1));
				memset(ipsec_msg2,0,sizeof(ipsec_msg2));
				memset(enc_str,0,sizeof(enc_str));
				memset(auth_str,0,sizeof(auth_str));
				...
				sprintf(ipsec_msg1,"IKE[\"%s\"] ESP_SA %s / %s / %d sec / SPI=[%08x:%08x]",st->st_connection->name,enc_str,auth_str,st->st_esp.attrs.life_seconds,ntohl(st->st_esp.our_spi),ntohl(st->st_esp.attrs.spi));
				send_log(ipsec_msg1);
				sprintf(ipsec_msg2,"IKE[\"%s\"] Set up ESP tunnel with %s Success!",st->st_connection->name,ip_str(&st->st_connection->that.host_addr));
				send_log(ipsec_msg2);
			  }
#endif	    
