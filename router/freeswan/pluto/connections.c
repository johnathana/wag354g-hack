 ...
 * for more details.
 *
 * RCSID $Id: connections.c,v 1.1.1.2 2005/03/28 06:57:43 sparq Exp $
 */
...
extract_end(struct end *dst, const struct whack_end *src, const char *which)
{
   ...
   dst->ca = empty_chunk;
	...
	else if (!streq(src->ca, "%any"))
	{
	    ...
	    if (ugh != NULL)
	    {
		log("bad CA string '%s': %s (ignored)", src->ca, ugh);
		dst->ca = empty_chunk;
	    }
	}
    }
...
, chunk_t requested_ca, bool initiator, bool aggrmode)
{
    ...
    for (p = pubkeys; p != NULL; p = p->next)
     ...
     && trusted_ca(peer_ca, c->that.ca, &peer_pathlen) && peer_pathlen == 0
     && trusted_ca(c->this.ca, requested_ca, &our_pathlen) && our_pathlen == 0)
	...
	for (; d != NULL; d = d->hp_next)
	{
	    ...
	    match = match_id(peer_id, &d->that.id, &wildcards)
                        ...
                        && trusted_ca(d->this.ca, requested_ca, &our_pathlen);
