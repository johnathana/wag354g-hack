 ...
 * for more details.
 *
 * RCSID $Id: spdb.c,v 1.1.1.3 2005/08/25 06:27:26 sparq Exp $
 */
...
struct db_attr oakley_tr_am111[] = {
...
struct db_attr oakley_tr_am112[] = {
	{ OAKLEY_ENCRYPTION_ALGORITHM, OAKLEY_DES_CBC },
	{ OAKLEY_HASH_ALGORITHM, OAKLEY_MD5 },
	{ OAKLEY_AUTHENTICATION_METHOD, OAKLEY_PRESHARED_KEY },
	{ OAKLEY_GROUP_DESCRIPTION, OAKLEY_GROUP_MODP1024 },
...
struct db_attr oakley_tr_am121[] = {
	{ OAKLEY_ENCRYPTION_ALGORITHM, OAKLEY_DES_CBC },
	{ OAKLEY_HASH_ALGORITHM, OAKLEY_SHA },
	{ OAKLEY_AUTHENTICATION_METHOD, OAKLEY_PRESHARED_KEY },
	{ OAKLEY_GROUP_DESCRIPTION, OAKLEY_GROUP_MODP768 },
...
struct db_attr oakley_tr_am122[] = {
	{ OAKLEY_ENCRYPTION_ALGORITHM, OAKLEY_DES_CBC },
	{ OAKLEY_HASH_ALGORITHM, OAKLEY_SHA },
	{ OAKLEY_AUTHENTICATION_METHOD, OAKLEY_PRESHARED_KEY },
	{ OAKLEY_GROUP_DESCRIPTION, OAKLEY_GROUP_MODP1024 },
...
struct db_attr oakley_tr_am511[] = {
	{ OAKLEY_ENCRYPTION_ALGORITHM, OAKLEY_3DES_CBC },
	{ OAKLEY_HASH_ALGORITHM, OAKLEY_MD5 },
	{ OAKLEY_AUTHENTICATION_METHOD, OAKLEY_PRESHARED_KEY },
	{ OAKLEY_GROUP_DESCRIPTION, OAKLEY_GROUP_MODP768 },
...
struct db_attr oakley_tr_am512[] = {
	{ OAKLEY_ENCRYPTION_ALGORITHM, OAKLEY_3DES_CBC },
	{ OAKLEY_HASH_ALGORITHM, OAKLEY_MD5 },
	{ OAKLEY_AUTHENTICATION_METHOD, OAKLEY_PRESHARED_KEY },
	{ OAKLEY_GROUP_DESCRIPTION, OAKLEY_GROUP_MODP1024 },
...
struct db_attr oakley_tr_am521[] = {
	{ OAKLEY_ENCRYPTION_ALGORITHM, OAKLEY_3DES_CBC },
	{ OAKLEY_HASH_ALGORITHM, OAKLEY_SHA },
	{ OAKLEY_AUTHENTICATION_METHOD, OAKLEY_PRESHARED_KEY },
	{ OAKLEY_GROUP_DESCRIPTION, OAKLEY_GROUP_MODP768 },
...
struct db_attr oakley_tr_am522[] = {
	{ OAKLEY_ENCRYPTION_ALGORITHM, OAKLEY_3DES_CBC },
	{ OAKLEY_HASH_ALGORITHM, OAKLEY_SHA },
	{ OAKLEY_AUTHENTICATION_METHOD, OAKLEY_PRESHARED_KEY },
	{ OAKLEY_GROUP_DESCRIPTION, OAKLEY_GROUP_MODP1024 },
...
//for bugs
struct db_attr oakley_tr_am525[] = {
	{ OAKLEY_ENCRYPTION_ALGORITHM, OAKLEY_3DES_CBC },
	{ OAKLEY_HASH_ALGORITHM, OAKLEY_SHA },
	{ OAKLEY_AUTHENTICATION_METHOD, OAKLEY_PRESHARED_KEY },
	{ OAKLEY_GROUP_DESCRIPTION, OAKLEY_GROUP_MODP1536 },
...
struct db_attr oakley_tr_am[4];

#endif
/* tables of transforms, (only one allowed for Aggressive Mode) */
//zhangbin 2004.10.28
#if 0
static struct db_trans oakley_trans_am[] = {
	{ KEY_IKE, AD(oakley_tr_am) },
    };
#else
static struct db_trans oakley_trans_am[1];
#endif
/* array of proposals to be conjoined (can only be one for Oakley) */
//zhangbin 2004.10.28
#if 0
static struct db_prop oakley_pc_am[] =
    { { PROTO_ISAKMP, AD(oakley_trans_am) } };
#else
    static struct db_prop oakley_pc_am[1];

#endif
/* array of proposal conjuncts (can only be one) */
#if 0
static struct db_prop_conj oakley_props_am[] = { { AD(oakley_pc_am) } };
#else
static struct db_prop_conj oakley_props_am[1];
#endif

/* the sadb entry */
//zhangbin
#if 0
struct db_sa oakley_sadb_am = { AD(oakley_props_am) };
#else
struct db_sa oakley_sadb_am;
#endif
...
static int cmp_enc_auth(const struct connection *c,u_int8_t enc,u_int16_t auth);//add by zhs for 3DES_bug,Mar 11,2005
#undef AD
#undef AD_NULL
...
, u_int8_t np)
{
	...
	for (pn = 0; pn != pc->prop_cnt; pn++)
	{
...
#ifdef ZB_DEBUG
	    {
		    ...
		    sprintf(tmpbuf,"pn=%d,proposal=%d,protoid=%d",pn,proposal.isap_proposal, proposal.isap_protoid);
		    send_log(tmpbuf);
	    }
#endif
...
#ifdef ZB_DEBUG
		{
			char tmpbuf[50];
			memset(tmpbuf,0,sizeof(tmpbuf));
			sprintf(tmpbuf,"tn=%d,np=%d,transform:transid=%d",tn,np,trans.isat_transid);
			send_log(tmpbuf);
		}
#endif
	...
	while (pbs_left(&trans_pbs) != 0)
	{
		...
		case OAKLEY_ENCRYPTION_ALGORITHM | ISAKMP_ATTR_AF_TV:
			...
			ta.encrypt = val;
			    ...
			    //zhangbin 2004.10.27
			{
				char buf[60];//zhs change 6 to 60
				memset(buf,0,sizeof(buf));
				sprintf(buf,"ta.encrypt=%d!!!!",val);
			    	send_log(buf);
			}
#endif
...
/* #endif */
   ...
   //sprintf(log_msg,"IKE[%s] :<FONT color=red>***Check your Encryption and Authentication and PFS method settings!</FONT><br>",st->st_connection->name); 
...
init_st_oakley(struct state *st, lset_t policy,char *ikebuf)
{
    struct oakley_trans_attrs ta;
   ...
   int mask = atoi(ikebuf); 
   size = sizeof(oakley_tr_am);
  //zhangbin 
#ifdef ZB_DEBUG
  send_log("mask!!"); 
  send_log(ikebuf);
	...
	{
		case 111:
			memcpy(oakley_tr_am,oakley_tr_am111,size);
			break;
		case 112:
			memcpy(oakley_tr_am,oakley_tr_am112,size);
			break;
		case 121:
			memcpy(oakley_tr_am,oakley_tr_am121,size);
			break;
		case 122:
			memcpy(oakley_tr_am,oakley_tr_am122,size);
			break;
		case 511:
			memcpy(oakley_tr_am,oakley_tr_am511,size);
			break;
		case 512:
			memcpy(oakley_tr_am,oakley_tr_am512,size);
			break;
		case 521:
			memcpy(oakley_tr_am,oakley_tr_am521,size);
			break;
		case 522:
			memcpy(oakley_tr_am,oakley_tr_am522,size);
			break;
		case 525:
			memcpy(oakley_tr_am,oakley_tr_am525,size);
			break;
	...
	oakley_trans_am[0].transid = KEY_IKE;
       	oakley_trans_am[0].attrs = oakley_tr_am;
	oakley_trans_am[0].attr_cnt = elemsof(oakley_tr_am);
	
	
	oakley_pc_am[0].protoid = PROTO_ISAKMP;
	oakley_pc_am[0].trans = oakley_trans_am;
	oakley_pc_am[0].trans_cnt = elemsof(oakley_trans_am);
	
	oakley_props_am[0].props = oakley_pc_am;
	oakley_props_am[0].prop_cnt = elemsof(oakley_pc_am);

	oakley_sadb_am.prop_conjs = oakley_props_am;
	oakley_sadb_am.prop_conj_cnt = elemsof(oakley_props_am);
...
#ifdef ZB_DEBUG 
    //zhangbin 2004.10.27
    sprintf(abuf,"life=%ld\n",ta.life_seconds);//zhs md for warning msg
    send_log(abuf);
#endif
    ...
    //ta.encrypt = OAKLEY_DES_CBC; /* OAKLEY_ENCRYPTION_ALGORITHM */
    //ta.encrypt = c->alg_info_ike->ike[0].ike_ealg;
    //ta.encrypt = oakley_tr_am1[0].val; /* OAKLEY_ENCRYPTION_ALGORITHM */
   
#ifdef ZB_DEBUG 
    //zhangbin 2004.10.27
    sprintf(abuf,"encrypt=%d!!!\n",ta.encrypt);
    send_log(abuf);
#endif
    
    ta.encrypter = crypto_get_encrypter(ta.encrypt);
    ta.enckeylen = ta.encrypter->keydeflen;
    //sprintf(abuf,"keylen=%d!!!",c->alg_info_ike->ike[0].ike_eklen);

#ifdef ZB_DEBUG
    send_log(abuf);
    sprintf(abuf,"ta.enckeylen=%d!!!",ta.enckeylen);
    send_log(abuf);
#endif

    //zhangbin 2004.10.27
    ta.hash = oakley_tr_am[1].val;  /* OAKLEY_HASH_ALGORITHM */
    //ta.hash = OAKLEY_MD5;  /* OAKLEY_HASH_ALGORITHM */
    //ta.hash = c->alg_info_ike->ike[0].ike_halg;  /* OAKLEY_HASH_ALGORITHM */
    //ta.hash = oakley_tr_am1[1].val;  /* OAKLEY_HASH_ALGORITHM */
    
    //zhangbin 2004.10.27
#ifdef ZB_DEBUG    
    sprintf(abuf,"hash=%d!!",ta.hash);
    send_log(abuf);
#endif
    ...
    //oakley_tr_am1[2].val = ta.auth; /* OAKLEY_AUTHENTICATION_METHOD */

    //zhangbin 2004.10.27
    ta.group = lookup_group(oakley_tr_am[3].val); /* OAKLEY_GROUP_DESCRIPTION */
    //ta.group = OAKLEY_GROUP_MODP768; /* OAKLEY_GROUP_DESCRIPTION */
    
    //ta.group = lookup_group(oakley_tr_am1[3].val); /* OAKLEY_GROUP_DESCRIPTION */
...
#ifdef ZB_DEBUG
    {
	    ...
	    sprintf(grpbuf,"Group=%d!!",oakley_tr_am[3].val);
	    send_log(grpbuf);
    }
#endif
    st->st_oakley = ta;
...
/***********************************************************************************
 *cmp_enc_auth:compare my encry and auth alg with another.
 *INPUT:
 *RETURN:OK    ---0,find the same
        ERROR  ---1,do not find the same
 *
 *modified hostory:
-------------------------------------------
01a  zhs write for 3DES bug,MAR 11,2005
                                                                                                                             
*/
static int cmp_enc_auth(const struct connection *c,u_int8_t enc,u_int16_t auth)
{
        int cnt = c->alg_info_esp->alg_info_cnt;
//      int is_find = 0;
        int i;
  ...
  #ifdef ZHS_DEBUG                                                                                   
        {
                ...
                sprintf(deb,"cmp_nec_auth with enc=%d,auth=%d\n",enc,auth);
                send_log(deb);
        }
        DB_ALG_INFO(c->alg_info_esp);
  #endif                                                                                                                           
        for(i=0;i<cnt;i++)
        {
                if( enc==c->alg_info_esp->esp[i].transid
                  &&auth==c->alg_info_esp->esp[i].auth)
                {
                        //is_find = 1;
			#ifdef ZHS_DEBUG
                        send_log("!!!cmp_nec_auth find,and ruturn OK!!!\n");
			#endif
                        return 0;
                }
        }
#ifdef ZHS_DEBUG
        send_log("cmp_nec_auth do not find and return ERROR\n");
#endif
        return 1;
}
...
DB_ALG_INFO(c->alg_info_esp);//add by zhs for debug,2005-3-11
  ...
  #ifdef ZHS_DEBUG                                                                                   
		{//add by zhs for debug
			char deb[50];
			memset(deb,0,sizeof(deb));
			sprintf(deb,"rec encry alg=%d,auth alg=%d\n",esp_attrs.transid,esp_attrs.auth);
			send_log(deb);
		}
#endif
                if(ESP_3DES==esp_attrs.transid)//add by zhs for 3DES bug,Mar 11,2005
                {
                        if(cmp_enc_auth(c,ESP_3DES,esp_attrs.auth))
                        {
                                send_log("not find proposal chosen for encry and auth alg!!!\n");
                                continue;
                        }
                }
    ...
    zhs_log(); 
   //sprintf(log_msg,"IKE[%s] :<FONT color=red>*** No Acceptable Proposal in IPsec SA</FONT><br>",st->st_connection->name);
    return NO_PROPOSAL_CHOSEN;
}
