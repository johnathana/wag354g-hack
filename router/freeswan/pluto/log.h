 ...
 * for more details.
 *
 * RCSID $Id: log.h,v 1.1.1.2 2005/03/28 06:57:44 sparq Exp $
 */
...
/* zhangbin for vpn-log */
extern int send_log(const char *message,...);
...
#define zhs_log() do{ \
		...
		sprintf(deb,"%s %s line%d..............\n",__FILE__,__FUNCTION__,__LINE__);\
		send_log(deb);\
		}while(0)	
#define DB_ALG_INFO(alg_info_esp) do{\
	 char deb[200];\
	 memset(deb,0,sizeof(deb));\
         sprintf(deb,"c.alg_info_esp info_cnt=%d,ref_cnt=%d,flags=%d,proto=%d\n" \
                                ,alg_info_esp->alg_info_cnt  \
                                ,alg_info_esp->ref_cnt       \
                                ,alg_info_esp->alg_info_flags \
                                ,alg_info_esp->alg_info_protoid);\
                        send_log(deb); \
			zhs_log();\
	memset(deb,0,sizeof(deb));\
        sprintf(deb,"esp[0].transid=%d,auth=%d,enckeylen=%d,authkeylen=%d,encryptalg=%d,authalg=%d\n" \
		,alg_info_esp->esp[0].transid \
		,alg_info_esp->esp[0].auth \
		,alg_info_esp->esp[0].enckeylen \
		,alg_info_esp->esp[0].authkeylen \
		,alg_info_esp->esp[0].encryptalg \
		,alg_info_esp->esp[0].authalg); \
		send_log(deb);\
		}while(0)
#else 
#define zhs_log() 
#define DB_ALG_INFO(aa)
#endif
