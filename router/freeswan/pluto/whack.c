 ...
 * for more details.
 *
 * RCSID $Id: whack.c,v 1.1.1.3 2005/08/25 06:27:26 sparq Exp $
 */
...
#   define CD_POLICY_FIRST  CD_PSK
    ...
    CD_NOTKEEPALIVE,/*add by zhs for keep-alive,2005-4-25*/
...
#   define OO	OPTION_OFFSET
    ...
    { "ca", required_argument, NULL, CD_CA + OO },
    ...
    { "notkeepalive", no_argument, NULL, CD_NOTKEEPALIVE + OO },/*add by zhs for keep-alive,2005-4-25*/ 
...
update_ports(struct whack_message * m)
{
    ...
    if (m->left.port != 0) {
        ...
        update_address_port(&m->left.client.addr, m->tunnel_addr_family, port);
    }
    if (m->right.port != 0) {
        port = htons(m->right.port);
        update_address_port(&m->right.host_addr, m->addr_family, port);
        update_address_port(&m->right.client.addr, m->tunnel_addr_family, port);
    }
}
...
main(int argc, char **argv)
{
	...
	*tunnel_af_used_by = NULL;
	...
	case CD_CA:	/* --ca <distinguished name> */
	...
	case CD_DONT_REKEY:	/* --donotrekey */
	    ...
	    continue;
        //add by zhs for keep-alive,2005-45-25
//      #ifdef KEEP_ALIVE
        case CD_NOTKEEPALIVE:
            msg.policy |=POLICY_NOT_KEEPALIVE;
            fprintf(stderr,"**********((set not keepalive and msg.policy=%llx))\n",msg.policy);
                continue;
//      #endif
        //end by zhs
    ...
    || !pack_str(&msg.ike)		/* string 11 */
    || !pack_str(&msg.esp)		/* string 12 */
