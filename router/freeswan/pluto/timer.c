 ...
 * for more details.
 *
 * RCSID $Id: timer.c,v 1.1.1.3 2005/08/25 06:27:26 sparq Exp $
 */
		...
		else
		{
			...
			, log("replacing stale %s SA"
			    , IS_PHASE1(st->st_state)? "ISAKMP" : "IPsec"));
		                    //add by zhs
//#ifdef KEEP_ALIVE
                    //if(st->st_policy & POLICY_NOT_KEEPALIVE)
                    if(IS_NOT_KEEPALIVE(st->st_policy))
                    {
                        char debug[100];
                        memset(debug,0,sizeof(debug));
                        sprintf(debug," 2nke[\"%s\"]: sa not replace......",st->st_connection->name);
                        send_log(debug);
                    }
                    else//end by zhs
//#endif
