 ...
 * for more details.
 *
 * RCSID $Id: preshared.c,v 1.1.1.3 2005/08/25 06:27:25 sparq Exp $
 */
...
#ifdef NOMAD_SUPPORT
const char *shared_secrets_file = "/tmp/ipsec_secrets/*.secret";/*modified by zhs*/
#else
...
process_secrets_file(const char *file_pat)
{
	...
	pos.filename = *fnp;
#else
	pos.filename = "/tmp/ipsec.secrets"; //modified by zhangbin
#endif
    ...
    globfree(&globbuf);//add by zhs for free mem, 3-23-2005
