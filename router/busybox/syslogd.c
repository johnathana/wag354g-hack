...
#include <sys/uio.h>
//Add by Lai for save log file at 2005-06-14 
#include <cy_conf.h>
//Add by lzh
#define MAX_LOG_SIZE        1024*32
#define SAVE_LOG_TIME       60*2
...
char *localip;
    ...
    struct time_zone_map time_map[]={
        {"kwajalein","-12 1 0","IDLW ","-12:00"},	
        {"Midway Island,Samoa",    "-11 1 0","NT   ","-11:00"},	
        {"Hawaii",   "-10 1 0","HST  ","-10:00"},	
        {"Alaska",   "-09 1 1","YST  ","-09:00"},	
        {"Pacific Time","-08 1 1","PST  ","-08:00"},	
        {"Arizona","-07 1 0","MST  ","-07:00"},	
        {"Mountain Time","-07 2 1","PDT  ","-07:00"},	
        {"Mexico","-06 1 0","CST  ","-06:00"},	
        {"Central Time","-06 2 1","MDT  ","-06:00"},	
        {"Indiana East,Colobia,Panama","-05 1 0","CDT  ","-05:00"},	
        {"Eastern Time","-05 2 1","ACT  ","-05:00"},	
        {"Bolivia","-04 1 0","ACST ","-04:00"},	
        {"Atlantic Time","-04 2 1","AST  ","-04:00"},	
        {"Newfoundland","-03.5 1 1","NFT  ","-03:30"},	
        {"Guyana","-03 1 0","AWT  ","-03:00"},	
        {"Brazil East,Greenland","-03 2 1","ADT  ","-03:00"},	
        {"Mid-Atlantic","-02 1 0","NDT  ","-02:00"},	
        {"Azores","-01 1 2","WAT  ","-01:00"},	
        {"Gambia,Liberia,Morocco","+00 1 0","WET  ","+00:00"},	
        {"England","+00 2 2","GMT  ","+00:00"},	
        {"Tunisia","+01 1 0","MEWT ","+01:00"},	
        {"France,Germany,Italy","+01 2 2","MEZ  ","+01:00"},	
        {"South Africa","+02 1 0","CEST ","+02:00"},	
        {"Greece,Ukraine,Romania,Turkey","+02 2 2","EET  ","+02:00"},	
        {"Iraq,Jordan,Kuwait","+03 1 0","BT   ","+03:00"},	
        {"ABU Dhabi,Muscat,Armenia","+04 1 0","SCT  ","+04:00"},	
        {"Pakistan,Russia","+05 1 0","IOT  ","+05:00"},	
        {"Bangladesh,Russia","+06 1 0","ALMT ","+06:00"},	
        {"Thailand,Russia","+07 1 0","CXT  ","+07:00"},	
        {"China,Hong Kong,Australia Western","+08 1 0","CCT  ","+08:00"},	
        {"Singapore,Taiwan,Russia","+08 2 1","WST  ","+08:00"},	
        {"Japan,Korea","+09 1 0","JST  ","+09:00"},	
        {"South Australia","+09.5 0 0","SAST ","+09:30"},	
        {"Guam,Russia","+10 1 0","GST  ","+10:00"},	
        {"Australia","+10 2 4","AEST ","+10:00"},	
        {"Solomon Islands","+11 1 0","AESST","+11:00"},	
        {"Fiji","+12 1 0","NZST","+12:00"},	
        {"New Zealand","+12 2 4","NZST ","+12:00"},	
...
//zhangbin
		...
		if(((p = strstr(msg_buf, "1n")) != NULL) /*||
		   ((p = strstr(msg_buf, "5r")) != NULL) ||
	       ((p = strstr(msg_buf, "5n")) != NULL) ||
		    ((p = strstr(msg_buf, "5b")) != NULL)*/)
		{
			msg = msg_buf + 2;
			message("%s %s\n", timestamp, msg);
		}
...
//				sprintf(msg_new,"<FONT color=red>%s</FONT>",msg_tmp);
		...
		//-----add by lzh 2004/6/24 to support upnp log------
		else if (((p = strstr(msg_buf,"5n"))!= NULL)||
				 ((p = strstr(msg_buf,"5r"))!= NULL)||
				 ((p = strstr(msg_buf,"5b"))!= NULL))
		{
			msg = msg_buf + 2;
...
static int serveConnection (int conn)
{
	...
	/* da_f 2005.8.9 add. Very important! */
	bzero(pkgsnmp, sizeof(pkgsnmp));
...
/////////////////////////////////1111//////////////////////////////////
		 ...
		 color=line[19];
         snmpflag=atoi(type); 
	     if((snmpflag==1)||(snmpflag==2)||(snmpflag==3)||(snmpflag==4)||(snmpflag==5)){  
	    ...
	    if((snmpflag==1)||(snmpflag==2)||(snmpflag==3)||(snmpflag==4)||(snmpflag==5)){  
			if((color=='r')||(color=='b')||(color=='n'))
			{
				...
				{   
					...
					{
						/* da_f 2005.8.4 modify. Change format 
							from: TCP: From: 10.0.0.1:1666 To: 100.0.0.1:21
							to  : @out TCP From 10.0.0.1:1666 To 100.0.0.1:21 
							or    @in TCP From 10.0.0.1:1666 To 100.0.0.1:21 */
						if((snmpflag == 3) || (snmpflag == 4))
						{
							if(pkg_i > 24)
							{
								if(pkg_i == 25)
								{
									/* Access Log. It's outgoing log. */
									if(snmpflag == 3)
									{
										/* append "@out " to pkgsnmp[pkg_j] */
										memcpy(&pkgsnmp[pkg_j], "@out ", 5);
										pkg_j += 5;
									}
									/* Firewall Log. It's incoming log. */
									else if(snmpflag == 4)
									{
										char *prefix = NULL;

										/* The prefix is "Hacker Attack" */
										prefix = strstr(&line[pkg_i], PREFIX_1);
										if(prefix)
										{
											/* append "@in " */
											memcpy(&pkgsnmp[pkg_j], "@in ", 4);
											pkg_j += 4;
									...
									/* skip whitespace we do not need */
									while(1)
									{
										if(line[pkg_i] != 0x20)
											break;
...
//written by lzh for save log to flash 2004/11/22
//static int nvram_match(char *name, char *match)
//{
//    const char *value=nvram_get(name);
//    return (value && !strcmp(value,match));
//}
static int SaveLogToFlash()
{
    int fd;
    struct stat flog;
    int fsize;
    ...
    if ((fd = device_open(logFilePath,O_RDONLY)) >= 0)
    {
        printf("111111111111111\n");
        //get the file information
        if(fstat(fd,&flog) == 0)
        {
            printf("current log file size=%d\n",flog.st_size);
            //if size > 32k,then save the last 32k log message to flash
            if (flog.st_size > MAX_LOG_SIZE)
            {
                int start_addr,size,offset;
                                                                                
                printf("size > Max\n");
                start_addr = size = offset = 0;
                                                                                
                fsize = MAX_LOG_SIZE;
                tmpbuf = malloc(fsize+1);
                memset(tmpbuf,0,fsize+1);
                                                                                
                offset = flog.st_size - fsize;
                start_addr = lseek(fd,offset,SEEK_SET);
                printf("seek start addr=%d,offset=%d\n",start_addr,offset);
                                                                                
                if (start_addr >= 0)
                //if ((start_addr = lseek(fd,fsize,SEEK_END)) >= 0)
                {
                        size = read(fd,tmpbuf,fsize);
                        if ((size != 0)&&(size != fsize)) return -1;
                        tmpbuf[fsize] = '\0';
                        //nvram_set("log_save",tmpbuf);
                }else
                    return -1;
            }else //if size < 32k,then save all log message to flash;
            {
                int readsize;
                                                                                
                printf("size < Max, then save full\n");
                readsize = 0;
                fsize = flog.st_size;
                tmpbuf = malloc(fsize+1);
                memset(tmpbuf,0,fsize+1);
                                                                                
                readsize = read(fd,tmpbuf,fsize);
                if ((readsize !=0)&&(readsize != fsize)) return -1;
                printf("readsize=%d,fsize=%d\n",readsize,fsize);
                tmpbuf[fsize] = '\0';
                //nvram_set("log_save",tmpbuf);
            }
            nvram_set("log_save",tmpbuf);
...
struct timeval SaveLogTime;
                                                                                
static void PreSaveLogMessage()
{
    printf("pre save system log to flash\n");
                                                                                
    gettimeofday(&CurrentTime,NULL);
    SaveLogTime.tv_sec = CurrentTime.tv_sec + 60;//SAVE_LOG_TIME;
    for(;;)
    {
        gettimeofday(&CurrentTime.tv_sec,NULL);
        if (CurrentTime.tv_sec >= SaveLogTime.tv_sec)
        {
            SaveLogTime.tv_sec = CurrentTime.tv_sec + 60;//SAVE_LOG_TIME;
            //sprintf(buf1,"lzh====%d\n",SaveLogTime.tv_sec);
            if(nvram_match("have_new_log","1"))
            {
                SaveLogToFlash();
                //nvram_set("log_save",buf1);
                //nvram_commit();
                nvram_set("have_new_log","0");
            }
        }
        if (nvram_match("log_enable","0")) break;
    }
    printf("Pre Save syslog over!\n");
}
...
extern int syslogd_main(int argc, char **argv)
{
    ...
    for(v =time_map  ; v < &time_map[38]; v++) {
    ...
    //add by lzh to save log to flash
	//Modify by Lai at 2005-06-14
#ifdef SAVE_LOG_SUPPORT
    child = fork();
	...
	if (child == 0)
    {
        printf("syslog:enter child pid\n");
        PreSaveLogMessage();
        exit(0);
    }
    printf("syslog:enter father pid\n");
#endif	
	doSyslogd();
...
int date_main_sys(int argc, char **argv)
{
    ...
    if (strncmp(date_fmt, "%f", 2) == 0) {
		...
		if(tm_time.tm_year<100){
	          date_fmt = "P%G-%m-%dT%H:%M:%S";
	            zone = 'a';
	          }
        else if(tm_time.tm_year>100){
	          date_fmt = "%G-%m-%dT%H:%M:%S";
	            zone = 'b';
	          }
...
*/			
    ...
    for(t_buff_i=0;t_buff_i<t_buff_len;t_buff_i++)
	   t_buff_all[t_buff_i]=t_buff[t_buff_i];
		 
	if(zone=='b'){
         t_buff_all[19]=abbre[0];
         t_buff_all[20]=abbre[1];
         t_buff_all[21]=abbre[2];
         t_buff_all[22]=abbre[3];
         t_buff_all[23]=abbre[4];
         t_buff_all[24]=abbre[5];
		 
       }
	else if(zone=='a')
	{

         for(t_buff_i=20;t_buff_i<25;t_buff_i++)
	          t_buff_all[t_buff_i]=' ';
