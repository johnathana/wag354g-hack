		...
		enum ip_conntrack_info ctinfo)
{
	...
	if (ctinfo != IP_CT_ESTABLISHED
		...
		//printk("Until there's been traffic both ways, don't look in packets.\n");
	...
	//printk("conntrack_ftp: datalen %u\n", datalen);
	//printk("conntrack_ftp: len %u\n", len);
		
	if ((datalen > 0) && (data[datalen-1] == '\n')) {
		//printk("conntrack_ftp: datalen %u ends in \\n\n", datalen);
		if (!old_seq_aft_nl_set
		    || after(ntohl(tcph->seq) + datalen, old_seq_aft_nl)) {
			//printk("conntrack_ftp: updating nl to %u = (SEQ (%u)+ DLEN (%u)\n",
	...
	if(!old_seq_aft_nl_set ||
		...
		//printk("ip_conntrack_ftp_help: wrong seq pos %s(old seq aft nl %u) tcp seq %u\n", old_seq_aft_nl_set ? "":"(UNSET) ", old_seq_aft_nl, ntohl(tcph->seq));
		return NF_ACCEPT;
