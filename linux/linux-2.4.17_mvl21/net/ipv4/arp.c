 ...
 *
 * Version:	$Id: arp.c,v 1.1.1.2 2005/03/28 06:57:08 sparq Exp $
 *
...
//cjg:2004-5-13;add for half bridge function
extern unsigned int wan_ip_addr;
extern unsigned int lan_ip_addr;
extern unsigned int half_bridge_enabled;
extern unsigned char lan_mac_addr;
...
#include <stb.h>
extern struct stb_dev_info stb_devinfo[MAX_STB_NUM];
...
extern unsigned int lan_ip_addr; /* lan_ip_addr  */
extern unsigned int lan_netmask; /* lan_netmask  */
#endif
/*
...
#ifdef CONFIG_STB_SUPPORT
        //junzhao 2004.9.1 stb support
        if((stb_enabled) && (stb_devnum) && (stb_netmask) && (stb_network))
           {
             if(arp->ar_op == __constant_htons(ARPOP_REQUEST))
                {
                        int found = 0;
			
                        if((!strcmp(dev->name, "br0") || !strcmp(dev->name, "eth0")) && find_stbinfo(IPMAC, sip, sha, NULL))
			{
				if(!find_stbinfo(IPONLY, tip, NULL, NULL))
					found = 1;
				/*
				if(find_stbinfo(IPONLY, tip, NULL, NULL))
					found = 0;
			...
			else if(((find_stbinfo(IPONLY, tip, tha, NULL))) &&
                         (((sip & lan_netmask) == (lan_ip_addr & lan_netmask)) || (( sip & stb_netmask) == (stb_network & stb_netmask))))
				found = 1;
			
			/*
			if(((find_stbinfo(IPONLY, tip, tha, NULL))) &&
                         (((sip & lan_netmask) == (lan_ip_addr & lan_netmask)) || (( sip & stb_netmask) == (stb_network & stb_netmask))))
                               found = 1;
                        else if((!strcmp(dev->name, "br0") || !strcmp(dev->name, "eth0")) && find_stbinfo(IPMAC, sip, sha, NULL) && !find_stbinfo(IPONLY, tip, NULL, NULL))
                               found = 1;
			*/
                        if(found)
                        {
                            my_printk("%08x -> %08x\n", sip, tip);
                            arp_send(ARPOP_REPLY, ETH_P_ARP, sip, dev, tip, sha, dev->dev_addr, dev->dev_addr);
                        }
                }
          }
#endif
...
{
		if(half_bridge_enabled){
			//printk("half bridge enabled\n");
			//printk("sip:%08x --tip:%08x -- wan_ip_addr:%08x\n",
				//sip, tip, htonl(wan_ip_addr));
			/*this arp request is generated by a lan CPE which have the wan side ip, or
			generated by a wan side device when the wan side is in 1483 bridge mode ;
			but when tip is euqal to wan ip address, and it is generated by a lan CPE want to share the wan ip a			    ddress, we need skip the regular process routine, just drop the packet 
			*/
			if(((sip == htonl(wan_ip_addr)) && (tip != htonl(wan_ip_addr))) /*|| (tip == htonl(lan_ip_addr)) || (sip == htonl(lan_ip_addr)) deleted by cjg;2005-5-31*/ || 
			  ((tip == htonl(wan_ip_addr)) && (memcmp(dev->name, "eth0", 4) &&  memcmp(dev->name, "br0", 4)))){
				if(arp->ar_op = __constant_htons(ARPOP_REQUEST)){
				//	printk("half bridged arp requery\n");
					arp_send(ARPOP_REPLY, ETH_P_ARP, tip, dev, tip, sha, dev->dev_addr, dev->dev_addr);
					//goto out;
				}
			}
			if((sip == htonl(wan_ip_addr)) && (tip == htonl(wan_ip_addr))){
				goto out;
			}
			...
			//	sha[4],
			//	sha[5]);
			if((sip != htonl(wan_ip_addr)) && (tip == htonl(wan_ip_addr)) && !memcmp((const void *)sha, (const void *)&lan_mac_addr, 6)){
				goto out;
			}
			if((tip == htonl(wan_ip_addr)) && (!memcmp(dev->name, "eth0", 4))){
				goto out;
			}
		}
}
