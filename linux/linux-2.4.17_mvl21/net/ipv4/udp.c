 ...
 *
 * Version:	$Id: udp.c,v 1.1.1.3 2005/08/25 06:27:04 sparq Exp $
 *
...
int udp_sendmsg(struct sock *sk, struct msghdr *msg, int len)
{
	...
	if (rt == NULL) {
		...
		//zhangbin
		zprintk("%s:daddr=%0x,sk->saddr=%0x,dport=%0x,sport=%0x\n",__FUNCTION__,daddr,sk->saddr,sk->dport,sk->sport);
	
		udp_srcport = sk->sport	;
		...
		//zhangbin
		zprintk("%s:rt->u.dst.dev->name=%s\n",__FUNCTION__,rt->u.dst.dev->name);
...
{
//add by zhs for IPSec,Dec10,2004
...
#endif//end by zhs for IPSec,Dec10,2004
...
extern unsigned char local_lan_mac_addr; 
int udp_rcv(struct sk_buff *skb)
{
	...
	UDP_INC_STATS_BH(UdpNoPorts);
		...
		if(half_bridge_enabled){
			...
			unsigned char *ptr;
			//unsigned short *type;
			int i = 0, pppoe_flag = 0;
			printk("skb->dev:%s\n", skb->dev->name);
			lan_dev = dev_get_by_name("br0");
			if(NULL != lan_dev){
				skb->len += (skb->nh.iph->ihl) * 4 + 14;
				if(strncmp(skb->dev->name, "ppp", 3)){
				...
				eth = skb->mac.raw;
				skb->dev = lan_dev;
				ptr = (unsigned char *)&lan_mac_addr;
				eth->h_dest[0] = *ptr;
				eth->h_dest[1] = *(ptr + 1);
				eth->h_dest[2] = *(ptr + 2);
				eth->h_dest[3] = *(ptr + 3);
				...
				eth->h_dest[5] = *(ptr + 5);
				if(pppoe_flag){
					ptr = (unsigned char *)&local_lan_mac_addr;
					eth->h_dest[6] = *ptr;
					eth->h_dest[7] = *(ptr + 1);
					eth->h_dest[8] = *(ptr + 2);
					eth->h_dest[9] = *(ptr + 3);
