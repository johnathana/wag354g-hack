 ...
 *
 * Version:	$Id: route.c,v 1.1.1.3 2005/08/25 06:27:04 sparq Exp $
 *
...
//#define ZB_DEBUG
#ifdef ZB_DEBUG
#define zprintk printk
#else
#define zprintk(format, args...) 
#endif
...
//zhangbin
extern __u16 udp_srcport;
extern unsigned int cloud_enabled;
#define DHCP_PORT	67
...
int ip_route_output_slow(struct rtable **rp, const struct rt_key *oldkey)
{
	...
	if (oldkey->oif) {
		...
		//zhangbin
		zprintk("%s:dev_out->name=%s!\n",__FUNCTION__,dev_out->name);
	...
	if (fib_lookup(&key, &res)) {
		...
		if (oldkey->oif) {
			...
			//zhangbin
			zprintk("2:in oldkey->oif!\n");
		
			if (key.src == 0)
			{
				//zhangbin
				zprintk("key.src = inet_select_addr!\n");
...
/* zhangbin 2005.6.15 */
	if(dev_out)
		zprintk("cloud_enabled=%d,dev_out->name=%s\n",cloud_enabled,dev_out->name);
	
	if (!key.src)
	{
		//zhangbin
		zprintk("1:udp_srcport=%0x!\n",udp_srcport);
		
		//zhangbin
		if((cloud_enabled) &&(dev_out) && (!strncmp(dev_out->name,"ipsec",5)) && (udp_srcport == htons(DHCP_PORT)))
		{
			//zhangbin
			zprintk("cloud_enabled:udp_srcport=%0x!\n",udp_srcport);
			
			{
				struct net_device *tmp_dev;
				struct in_device *tmp_in_dev;
				u32 addr = 0;
			
				tmp_dev = dev_get_by_name("br0");
				read_lock(&inetdev_lock);
				tmp_in_dev = __in_dev_get(tmp_dev);
				...
				read_lock(&tmp_in_dev->lock);
				for_primary_ifa(tmp_in_dev)
				{
					if(!key.dst||inet_ifa_match(key.dst,ifa))
					{
						addr = ifa->ifa_local;
						break;
					}
				...
				}endfor_ifa(tmp_in_dev);

				read_unlock(&tmp_in_dev->lock);
				read_unlock(&inetdev_lock);
	...
	/* zhangbin 2005.6.15 */
	zprintk("dev_out->name=%s!\n",dev_out->name);
	/* for SNMP accessing from VPN tunnel*/	
	if((cloud_enabled) &&(dev_out) && (!strncmp(dev_out->name,"ipsec",5)) && (udp_srcport == htons(SNMP_PORT)))
	{
		struct net_device *tmp_dev;
		struct in_device *tmp_in_dev;
		u32 addr = 0;
	...
	for (rth = rt_hash_table[hash].chain; rth; rth = rth->u.rt_next) {
		if (rth->key.dst == key->dst &&
			...
			*rp = rth;
			...
			//zhangbin
			zprintk("%s:key->dst=%0x,key->src=%0x,rth->rt_dst=%0x,rth->rt_src=%0x,name=%s\n",__FUNCTION__,key->dst,key->src,rth->rt_dst,rth->rt_src,rth->u.dst.dev->name);
