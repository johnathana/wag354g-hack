...
extern unsigned char wan_dev_name;
extern unsigned int bridge_only_enabled;
extern unsigned int pppoe_pass_enabled;
...
/* add for private errno num  2005.2.2 */
#define THROUGH     0      //pppoe passthrough ok
#define ENORMAL     1      //normal handler
#define EDROPPACK   2      //drop packet

#ifdef TI_SLOW_PATH
int pppoe_handler(struct sk_buff *skb)
{		
	struct ethhdr *eth;
	struct net_device *extern_dev;
	struct net_device *lan_dev;

	extern_dev = dev_get_by_name(&wan_dev_name);
		
	if(extern_dev == NULL)
	{
		//printk("can not get the extern_dev\n");
		return -ENORMAL;
	}
	eth = skb->mac.ethernet;
	if((ntohs(eth->h_proto) == ETH_P_PPP_DISC 
	   || ntohs(eth->h_proto) == ETH_P_PPP_SES) 
	   && (memcmp(eth->h_dest, extern_dev->dev_addr, 6)))
	{
	       /* printk("eth->h_dest:%02x:%02x:%02x:%02x:%02x:%02x\n",
			eth->h_dest[0],eth->h_dest[1],eth->h_dest[2],
			eth->h_dest[3],eth->h_dest[4],eth->h_dest[5]);*/
		lan_dev = dev_get_by_name("eth0");
		if(memcmp(eth->h_dest, lan_dev->dev_addr, 6) == 0)
		{
			/* pppoe session to access device's service, if this is accepted?*/
			//printk("pppoe session access device's service\n");
			kfree_skb(skb);
			dev_put(extern_dev);/* dec ref count */
			return -EDROPPACK;
		}
		...
		else if(!strcmp(skb->dev->name, &wan_dev_name))
		{
			//printk("wan2lan\n");
			skb->dev = dev_get_by_name("eth0");
			dev_put(skb->dev);            /* dec ref count */
		}
		else
		{
			//printk("no dev name match!\n");
			kfree_skb(skb);
			dev_put(extern_dev); /* dec ref count */
			return -EDROPPACK;
		}
...
//junzhao 2005.2.22 add for pppoe_passthrough not define TI_SLOW_PATH 
#else  
...
{		
	struct net_device *extern_dev;
	struct net_device *lan_dev;
	unsigned char *tmpdata = skb->data;
	unsigned int tmplen = skb->len;
	...
	if(!strcmp(skb->dev->name, &wan_dev_name))
	{
		tmpdata -= 14 ;
		tmplen = skb->len + 14;
	}
	
	if((tmpdata[12] == 0x88) && (tmpdata[13] == 0x63 || tmpdata[13] == 0x64) && (maccmp(tmpdata, extern_dev->dev_addr)))
	{
		lan_dev = dev_get_by_name("eth0");
		if(!maccmp(tmpdata, lan_dev->dev_addr))
		{
			/* pppoe session to access device's service, if this is accepted?*/
			//printk("pppoe session access device's service\n");
			kfree_skb(skb);
...
int netif_rx(struct sk_buff *skb)
{
	...
	//junzhao 2005.2.2 for pppoe passthrough check!!
	int ret;	
	if((!bridge_only_enabled) && pppoe_pass_enabled 
	     && (ret = pppoe_handler(skb)) != -ENORMAL)
		return 0;

	//printk("pppoe_handler result %d\n", ret);
