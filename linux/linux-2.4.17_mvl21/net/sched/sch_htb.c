 ...
 *
 * $Id: sch_htb.c,v 1.1.1.3 2005/08/25 06:27:05 sparq Exp $
 */
...
struct htb_sched
{
    struct htb_realTime realTime_bw[10];     //add by csh for qos. 2005.07.27
...
static __inline__ struct htb_class *htb_find(u32 handle, struct Qdisc *sch)
{
	...
	if (TC_H_MAJ(handle) != sch->handle) 
		//说明handle参数是空值或其代表的类不是sch的一个子类
		return NULL;
	//在sch结构中根据handle参数hash出相应的类
...
static struct htb_class *htb_classify(struct sk_buff *skb, struct Qdisc *sch)
{
	...
	/* allow to select class by setting skb->priority to valid classid;
	   ...
	   rules in it */
	   /*若由其他的方式如FW等，而不是由TC FILTER来给skb分类
	      此时就要根据skb->priority的值来查找类。
	      */
	if (skb->priority == sch->handle)   /*skb->priority = X:0的情况*/
		return HTB_DIRECT;  /* X:0 (direct flow) selected */
	if ((cl = htb_find(skb->priority,sch)) != NULL) /*skb->priority = X:P且P>0的情况*/
		return cl;

	/*以下是根据TC FILTER来分类的情况，则根据skb的IP地址等
		查找相应的类*/
	tcf = q->filter_list;
		...
		struct htb_class *cl,int prio)
{
...
#endif
	while (*p) {  /*根据新加入类的classid的值确定其排列的位置*/
	...
	rb_link_node(&cl->node[prio], parent, p);  /*将新类cl加入q->row[][]*/
		...
		struct htb_class *cl,int mask)
{
	...
	HTB_CHCL(cl);
	q->row_mask[cl->level] |= mask;/*在q->row_mask[]中记入该类的优先级值*/
	while (mask) {
		int prio = ffz(~mask);
		mask &= ~(1 << prio);
		/*将该类加入q->row[][]*/
...
static void htb_activate_prios(struct htb_sched *q,struct htb_class *cl)
{
	...
	/*一个叶子类在开始有包入队是，其cmode == HTB_CAN_SEND;
	  mask = cl->prio_activity的值在htb_activate函数中赋值*/
...
static int htb_enqueue(struct sk_buff *skb, struct Qdisc *sch)
{
    ...
    DEVIK_MSTART(0);
    if (cl == HTB_DIRECT || !cl) {   /*cl==-1或者cl==0*/
	/* enqueue to helper queue */
	if (q->direct_queue.qlen < q->direct_qlen && cl) {
		/*cl==-1并且X:0未满，则让数据包进入X:0队列*/
...
int tc_calc_rtable(unsigned bps,  __u32 *rtab, int cell_log, unsigned mtu, unsigned mpu)
{
	...
	for (i=0; i<256; i++) {
		...
		if (sz < mpu)
			sz = mpu;
		rtab[i] = 1000000 * sz/bps;
	}
	return cell_log;
}

//add by csh for qos. 2005.07.27
static int calculate_for_class(struct htb_class *cl)
{
	int mtu;
	unsigned buffer, cbuffer;
	mtu = 1600;
	buffer = cl->rate->rate.rate / HZ + mtu;
	cbuffer = cl->ceil->rate.rate / HZ + mtu;

	if ((cl->rate->rate.cell_log = tc_calc_rtable(cl->rate->rate.rate,cl->rate->data, -1, mtu, 0)) < 0) {
		return -1;
	}
	cl->buffer = 1000000 * buffer/(cl->rate->rate.rate);
	
	if ((cl->ceil->rate.cell_log = tc_calc_rtable(cl->ceil->rate.rate, cl->ceil->data, -1, mtu, 0)) < 0) {
		return -1;
	}
	cl->cbuffer = 1000000 * cbuffer/(cl->ceil->rate.rate);
}
...
htb_dequeue_tree(struct htb_sched *q,int prio,int level)
{
	...
	for(i=0; i<10; i++)
	{
		if(cl->classid == 0) // strange condition,i must depend self.
			break;
		if(q->realTime_bw[i].classid == 0)
		{
			//first time transfer packets, record something of it.
			q->realTime_bw[i].classid = cl->classid;
			q->realTime_bw[i].ceil = cl->ceil->rate.rate;
			q->realTime_bw[i].flag = 1;
			q->realTime_bw[i].time = jiffies + HZ * 2;
			q->realTime_bw[i].cl = cl;
			reallocate_flag = 1;
			break;
		}
		if(q->realTime_bw[i].classid == cl->classid)
		{
			//we have records of this class, is it active?
			if(q->realTime_bw[i].flag == 0)
			{
				//not active now, so this time it transfer packtes, we flag it as active.
				q->realTime_bw[i].flag = 1;
				q->realTime_bw[i].time = jiffies + HZ * 2;
				reallocate_flag = 1;
			}
	...
	for(i=0; i<10; i++)
	{
		//this class hasn't tranferred packets for 2s, must be inactive
		if(q->realTime_bw[i].flag == 1 && time_after(jiffies, q->realTime_bw[i].time))
		{
			q->realTime_bw[i].flag = 0;
			reallocate_flag = 1;
		}
	}
	//now reallocate bandwidth for each active class.
	if(reallocate_flag == 1)
	{
		//if have queues to transfer packets
		reallocate_flag = 0;
		for(i=0; i<10; i++)
		{
			if(q->realTime_bw[i].flag == 1)
				reallocate_flag = 1;
		}
		//real time reallocate bandwidth
		if(reallocate_flag == 1)
		{
			//printk("/////////////begin to calculate/////////\n");
			sum = 0;
			for(i=0; i<10; i++)
				if(q->realTime_bw[i].flag == 1)
					sum += q->realTime_bw[i].ceil;
			for(i=0; i<10; i++)
			{
				if(q->realTime_bw[i].flag == 1)
				{
					//recount each class's ceil
					q->realTime_bw[i].cl->ceil->rate.rate = (q->realTime_bw[i].cl->rate->rate.rate / sum) * q->realTime_bw[i].ceil;
					//printk("================2\n");
					//printk("cl.classid=%0x\n", q->realTime_bw[i].cl->classid);
					//printk("cl.rate=%d\n", q->realTime_bw[i].cl->rate->rate.rate);
					//printk("stand_ceil=%d\n", q->realTime_bw[i].ceil);
					//printk("sum=%d\n", sum);
					//printk("cl.ceil=%d\n", q->realTime_bw[i].cl->ceil->rate.rate);
					calculate_for_class(q->realTime_bw[i].cl);
				}
			}
		}
	}
...
{
	struct htb_sched *q = (struct htb_sched*)sch->data;/*q 指向sch->data*/
