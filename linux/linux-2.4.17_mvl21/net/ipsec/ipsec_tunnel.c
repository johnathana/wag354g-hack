...
char ipsec_tunnel_c_version[] = "RCSID $Id: ipsec_tunnel.c,v 1.1.1.3 2005/08/25 06:27:03 sparq Exp $";
...
/*	struct device *virtdev; */
	...
	//zhangbin
	zprintk("enter ipsec_tunnel_start_xmit!\n");
	{
		int i;
		for(i = 0;i < 60;i++)
		{
	...
	if (ip_chk_addr((unsigned long)iph->saddr) == IS_MYADDR
		...
		//zhangbin
		zprintk("outgoing_said = er->er_said!\n");
			...
			case SPI_TRAPSUBNET:
			{
				...
				if (pfkey_acquire(&tdb) == 0) {
					...
					if (outgoing_said.spi==htonl(SPI_TRAPSUBNET)) {
						...
						spin_unlock(&eroute_lock);
					} else if (create_hold_eroute(skb, iph, eroute_pid)) /* --zhangbin */ {
						skb = NULL;
					}
				}
				stats->tx_dropped++;
...
/*                            tailroom += ((blocksize - ((pyldsz + 2 * sizeof(unsigned char)) % blocksize)) % blocksize) + 2; */
	...
	if ((natt_type) && (outgoing_said.proto != IPPROTO_IPIP)) {
		...
		//zhangbin
		zprintk("skb->len < hard_header_len!\n");
 ...
 *
 * Revision 1.1.1.3  2004/10/28 09:02:31  sparq
