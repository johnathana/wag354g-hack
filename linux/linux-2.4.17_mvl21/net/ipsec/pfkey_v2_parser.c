 ...
 * for more details.
 *
 * RCSID $Id: pfkey_v2_parser.c,v 1.1.1.3 2005/08/25 06:27:03 sparq Exp $
 */
...
char pfkey_v2_parser_c_version[] = "$Id: pfkey_v2_parser.c,v 1.1.1.3 2005/08/25 06:27:03 sparq Exp $";
...
pfkey_safe_build(int error, struct sadb_ext *extensions[SADB_MAX+1])
{
	...
	if (!error) {
		KLIPS_PRINT(debug_pfkey, "klips_debug:pfkey_safe_build:"
			    "success.\n");
	//zhangbin
	//printk("pfkey_safe_build-success!\n");
		return 1;
	} else {
		KLIPS_PRINT(debug_pfkey, "klips_debug:pfkey_safe_build:"
			    "caught error %d\n",
...
//zhangbin
	//printk("will pfkey_extensions_init!\n");
	pfkey_extensions_init(extensions);

//zhangbin
	//printk("after pfkey_extensions_init!\n");
	...
	if((satype == 0) || (satype > SADB_SATYPE_MAX)) {
		KLIPS_PRINT(debug_pfkey, "klips_debug:pfkey_acquire: "
			    ...
			    satype);
		//zhangbin
		//printk("SAtype=%d unspecified or unknown.\n",satype);
		SENDERR(EINVAL);
	}
//zhangbin
	//printk("will pfkey_registered_sockets!\n");
	if(!(pfkey_registered_sockets[satype])) {
		KLIPS_PRINT(debug_pfkey, "klips_debug:pfkey_acquire: "
			    "no sockets registered for SAtype=%d(%s).\n",
			    satype,
			    satype2name(satype));
		//zhangbin
		//printk("no sockets for SAtype=%d(%s).\n",satype,satype2name(satype));
		SENDERR(EPROTONOSUPPORT);
	}
		
	//zhangbin
	//printk("will pfkey_safe_buile!\n");
	...
	//zhangbin
	//printk("pfkey_acquire:failed to build the acquire message\n");
		SENDERR(-error);
	}
	...
	for(pfkey_socketsp = pfkey_registered_sockets[satype];
		...
		if((error = pfkey_upmsg(pfkey_socketsp->socketp, pfkey_msg))) {
			KLIPS_PRINT(debug_pfkey, "klips_debug:pfkey_acquire: "
				    ...
				    error); 
			//zhangbin
			/*printk("acquire:up message for satype=%d(%s) to socket=%p failed with error=%d.\n",
				    satype,
				    satype2name(satype),
				    pfkey_socketsp->socketp,
				    error);*/
 ...
 * Revision 1.1.1.3  2005/08/25 06:27:03  sparq
 * GuangZhou release v1.01.08.
 *
 * Revision 1.1.1.2  2004/10/28 09:02:31  sparq
 * Mirror v1.00.11 release from G.Z division of CyberTAN.
 *
