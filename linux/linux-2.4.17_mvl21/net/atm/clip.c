...
static int clip_start_xmit(struct sk_buff *skb,struct net_device *dev)
{
	...
	if (entry->vccs->encap) {
		void *here;
/* zhangbin */
		DPRINTK("%s,will skb_push------!\n",__FUNCTION__);
		DPRINTK("skb data %p,head %p,",skb->data,skb->head);
		//xdump(skb->head,skb->len,"skb head");//add by zhs only for debug
		//xdump(skb->data,64,"skb data");//add by zhs only for debug
//add by zhs for add headroom,2005-6-16
		if(skb_headroom(skb)< RFC1483LLC_LEN)
		{
			struct sk_buff *n=skb_realloc_headroom(skb, RFC1483LLC_LEN);
			if(NULL==n )
			{
				printk(KERN_ERR "%s skb_realloc_headroom failed!\n");
				kfree_skb(skb);
				return 0;
