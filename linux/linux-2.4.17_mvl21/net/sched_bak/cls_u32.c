 ...
 * to do the following:
 * 		 The function u32_classify has been modified to mangle the packet with a new 
 * 		 TOS value. Also modified the same function to support port range. Added a new
 * 		 function u32_port_match for the same purpose.
 *
...
static int u32_port_match(u8 *ptr, struct tc_u32_key *key, u32 off2)
{
	__u16 src_min, src_max, dst_min, dst_max;
	__u16 src_port, dst_port;
	__u32 value;

	/* Get min/max values for source and destination ports.
	 * Since the key->val and key->mask are in network order, 
	 * the 1st u16 values are dst_min and dst_max */	
	dst_min = ntohs((key->val & 0xffff0000) >> 16);
	src_min = ntohs(key->val & 0x0000ffff);
	dst_max = ntohs((key->mask & 0xffff0000) >> 16);
	src_max = ntohs(key->mask & 0x0000ffff);

	/* Get source and destination port from the packet.
	 * Since value is in the network order, 1st u16 value is dst_port. */
	value = (*(u32*)(ptr+key->off+(off2&key->offmask)));
	dst_port = ntohs((value & 0xffff0000) >> 16);
	src_port = ntohs(value & 0x0000ffff);

	/* The max value for src/dst is checked if it is zero, then 
	 * corresponding classification is ignored. */
	if (((src_max != 0) && (src_port < src_min || src_port > src_max)) ||
		((dst_max != 0) && (dst_port < dst_min || dst_port > dst_max)))
...
static int u32_classify(struct sk_buff *skb, struct tcf_proto *tp, struct tcf_result *res)
{
    ...
    /* This can happen for PPP Control packets. These packets are not IP packets so the
     * IP header is NULL. This packet is classified to the EF2 queue.*/
    if (ptr == NULL)
    {
        res->classid = 2;
        return 0; 
    ...
    chk = skb->nh.iph->check;

#ifdef CONFIG_IP_NF_TARGET_MASQUERADE
	src = skb->nh.iph->saddr;
	skb->nh.iph->saddr = *(__u32 *)skb->cb;
#endif
	...
	skb->nh.iph->check = (u16)skb->__unused;
...
next_knode:
	if (n) {
		...
		for (i = n->sel.nkeys; i>0; i--, key++) {
            ...
            {
        		skb->nh.iph->check = 0;
                skb->nh.iph->tos = key->val >> 8; /* Write both Precedence and TOS bits */
                tos_set = 1;
            }
			else if (key->off == 20)
			{
			   	if (!u32_port_match(ptr, key, off2)) {
					n = n->next;
					goto next_knode;
				}
			}
			else if ((*(u32*)(ptr+key->off+(off2&key->offmask))^key->val)&key->mask) {
				n = n->next;
				goto next_knode;
			}
		}
