...
//cjg:2004-5-15;add for half bridge function
...
//cjg:2004-5-15;add for half-bridge function
	...
	if(half_bridge_enabled){
		if((key->dst == htonl(wan_ip_addr))&&
	            ((ntohl(key->src) & lan_netmask ) == (lan_ip_addr & lan_netmask))){
			if(main_table->tb_lookup(main_table, key, res) &&
			   local_table->tb_lookup(local_table, key, res)){
				return -ENETUNREACH; /* can not find the item in the main table */
			}else{
				goto out; /* find the item in main table */
			}
		}
	}
#endif
