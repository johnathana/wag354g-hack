...
#ifdef CONFIG_TI_CUSTOM_SYS_CALLS
/*
  MIPS has no standard implementation of syscall. In order to provide custom
  syscall the ti_syscall function is implemented here
*/
ti_syscall_t ti_syscalls[NR_TI_SYSCALLS];

asmlinkage int sys_ti_custom(int nfunc, void* param, int *ret_val)
{

  if ((nfunc < 0)||(nfunc >= NR_TI_SYSCALLS))
    return -ENOSYS;

  if (ti_syscalls[nfunc] == NULL)
    return -ENOSYS;

  *ret_val = (ti_syscalls[nfunc])(param);
