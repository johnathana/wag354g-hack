...
Int32 cslMarvell6063Start (PCSL_SWITCH_DRV_HANDLE swHead)
{
    ...
    swHead->numberOfPorts = 7;
    regaddr = 0x4;

    for (portNum = 0; portNum < swHead->numberOfPorts;portNum++)
    {
...
Int32 cslMarvell6060Start (PCSL_SWITCH_DRV_HANDLE swHead)
{
    Int32 retVal = -1;
    Uint32 regaddr,phyaddr,data;
    Uint32 portNum;
...
*
* INPUTS:
*     [In]    PSWITCH_DRV_HANDLE pDrvHead,
*     [In]    Uint32            port
* 
*
*
...
Int32 cslMarvell6063PrepareReadStatSequence(PCSL_SWITCH_DRV_HANDLE swHead,Uint32 port)
{
    Uint32 data;
...
Int32 cslMarvell6063GetPreparedCounters(PCSL_SWITCH_DRV_HANDLE swHead,Uint32 statIndex,Uint32* counter)
{
    Uint32 data,cnt;
    Uint32 regaddr,phyaddr;
    ...
    regaddr = 0x1E;
    phyRegRead(swHead, regaddr, phyaddr, readData1);

    regaddr = 0x1F;
    phyRegRead(swHead, regaddr, phyaddr, readData2);

    *counter = PAL_MK_UNIT32(readData1,readData2);
...
void cslMarvell6063SwitchGetVersion(PCSL_SWITCH_DRV_HANDLE swDrv,PSW_VER_INFO pSwitchInfo)
{
    ...
    pSwitchInfo->maxNumberOfPorts       = 7;
    pSwitchInfo->maxNumberOfPriorities  = 4;
    pSwitchInfo->maxNumberOfVLANs       = 64;

    phyRegRead(swDrv, regaddr, phyaddr, pSwitchInfo->Model);
     phyaddr = PORT_PHY_ADDR(0x5);
     regaddr = 0x4;
    phyRegRead(swDrv, regaddr, phyaddr, data);
    PAL_PRINT("regaddr = 0x4 value = 0x%x \r\n",data);

    regaddr = 0x6;
            for (port = 0; port < swDrv->numberOfPorts;port++)
        {
            phyaddr = PORT_PHY_ADDR(port);
            
            phyRegRead(swDrv, regaddr, phyaddr, data);
            PAL_PRINT("regaddr = 0x6 value = 0x%x \r\n",data);
...
void cslMarvell6060SwitchGetVersion(PCSL_SWITCH_DRV_HANDLE swDrv,PSW_VER_INFO pSwitchInfo)
{
    Uint32 regaddr,phyaddr;
    ...
    pSwitchInfo->is802_1_P_capable      = PAL_True; 
    pSwitchInfo->is802_1_Q_capable      = PAL_True;
    pSwitchInfo->maxNumberOfPorts       = 6;
...
Int32 cslMarvell6063VlanDefaultSet(PCSL_SWITCH_DRV_HANDLE swHead, Uint32 portNum,PCSL_SWITCH_VLAN_TABLE pVlnEntry)
{
    Uint32 regaddr,phyaddr,data;
...
Int32 cslMarvell6063VlanUpdate(PCSL_SWITCH_DRV_HANDLE swHead, PCSL_SWITCH_VLAN_TABLE pVlnEntry)
{
    Uint32 regaddr,phyaddr,portMask,idx,dbNum;
    Uint32 data = 0;
    ...
    if (pVlnEntry->valid)
    {
        ...
        for(data = 0;idx < swHead->numberOfPorts; idx++,portMask >>= 1)
        {
            data |= (PORT_MEMBER_MASK(portMask,pVlnEntry->portTagMask >> (idx - 4)) ) << ((idx - 4)*4);
        }
...
Int32 cslMarvell6063PrioritySet(PCSL_SWITCH_DRV_HANDLE swHead, Uint32 portNum, Uint32 priority)
{
    Uint32 regaddr,phyaddr,data;
...
Uint32 cslMarvell6063PriorityGet(PCSL_SWITCH_DRV_HANDLE swHead, Uint32 portNum)
{
    Uint32 regaddr,phyaddr,data;
...
/* all the ports are ingress checkin/ no ingress checking */
    ...
    case OPER_INGRESS:            
        ...
        for (port = 0; port < swHead->numberOfPorts;port++)
        {
            phyaddr = PORT_PHY_ADDR(port);
    ...
    case OPER_INGRESS_OFF:
        ...
        for (port = 0; port < swHead->numberOfPorts;port++)
        {
           ...
           phyaddr = PORT_PHY_ADDR(port);
           data = (data & (~(0x01 << port )) );
           PAL_PRINT("Write to Port # = 0x%x the value = 0x%x \r\n",port,data);
    ...
    case OPER_PORT_RECOGNITION_ON:   
        phyaddr = PORT_PHY_ADDR(swHead->initParams.internalPortNum);
        regaddr = 0x4;
        phyRegRead(swHead, regaddr, phyaddr, data);
        if(!arg)
            data &= 0xBFFF;
...
Int32 cslMarvell6060SpecialCapsSet(PCSL_SWITCH_DRV_HANDLE swHead, Uint32 cmd, Uint32 arg)
{
    Uint32 regaddr,phyaddr,data;                    
...
Int32 cslMarvell6063PurgeAtuEntry(PCSL_SWITCH_DRV_HANDLE swHead, char MacAddress[6])
{
    Uint32 regaddr,phyaddr;
    Uint32 data = 0;
   ...
   /* ATU MAC ADDRESS Register Bytes 0 & 1 */
    ...
    regaddr = 0x0D;
    PAL_PRINT("Bytes 0 & 1 regaddr = 0x%x value = 0x%x \r\n",regaddr,data1);
    
    phyRegWrite(swHead, regaddr, phyaddr,(Uint32)data1);
    
   /* ATU MAC ADDRESS Register Bytes 2 & 3 */

    data1 = ((MacAddress[2] << 8) | MacAddress[3] );     
    regaddr = 0x0E;
    PAL_PRINT("Bytes 2 & 3 regaddr = 0x%x value = 0x%x \r\n",regaddr,data1);
    ...
    PAL_PRINT("Bytes 4 & 5 regaddr = 0x%x value = 0x%x \r\n",regaddr,data1);
