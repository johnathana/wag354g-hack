...
static int i8k_get_info(char *buffer, char **start, off_t fpos, int length)
{
    ...
    cpu_temp     = i8k_get_cpu_temp();			/* 11100 큦 */
    left_fan     = i8k_get_fan_status(I8K_FAN_LEFT);	/*   580 큦 */
    right_fan    = i8k_get_fan_status(I8K_FAN_RIGHT);	/*   580 큦 */
    left_speed   = i8k_get_fan_speed(I8K_FAN_LEFT);	/*   580 큦 */
    right_speed  = i8k_get_fan_speed(I8K_FAN_RIGHT);	/*   580 큦 */
    fn_key       = i8k_get_fn_status();			/*   750 큦 */
    if (power_status) {
	ac_power = i8k_get_power_status();		/* 14700 큦 */
    } else {
	ac_power = -1;
    }
