...
#if defined(CONFIG_TI_FILESYSTEM)
#include <asm/mips-boards/prom.h>
extern char *prom_getenv(char *name);
#endif /* CONFIG_TI_FILESYSTEM */
...
#if defined(CONFIG_TI_FILESYSTEM)
static void tirg_load_image(int);
typedef unsigned char  uch;
static char *input_data;
static uch *flash;
static char *fs_ptr_s;
static int ticrd_load(uch *indata, struct file *outfp);
void rd_load(void);
#else /* !CONFIG_TI_FILESYSTEM*/
...
#ifdef RD_LOADER
#if defined(CONFIG_TI_FILESYSTEM)
static void __init tirg_load_image(int unit)
{
	...
	init_special_inode(out_inode, S_IFBLK | S_IRUSR | S_IWUSR,
	...
	if (nblocks == 0) {
          ...
          /* get base address of flash file system from environment */
          fs_ptr_s = prom_getenv("linux_fs_start");
          if(fs_ptr_s == NULL)
            {
              printk("RAMDISK ERROR: linux_fs_start not set in enviroment memory\n");
              goto free_inode;
            }

          else
            flash = simple_strtol(fs_ptr_s,NULL,0);

          if(!flash)
            {
              printk("RAMDISK ERROR: Invalid linux_fs_start\n");
              goto free_inode;
            }


          printk("Loading compressed image TI RG (0x%x) \n",flash);

           if (ticrd_load(flash, &outfile) == 0)  /* Need to enter address of flash image */
              goto successful_load;

#else /* !BUILD_CRAMDISK */
           printk(KERN_NOTICE                         /* The rest of this is for non-compressed img */
...
successful_load:
        printk("Ram Disk successfully loaded with TI-RG image \n");
...
static void __init rd_load_image(kdev_t device, int offset, int unit)
{
	...
	char rotator[4] = { '|' , '/' , '-' , '\\' };
#endif /* !CONFIG_ARCH_S390 && !CONFIG_PPC_ISERIES */
	ram_device = MKDEV(MAJOR_NR, unit);
...
#if defined(CONFIG_TI_FILESYSTEM)
static int __init
ticrd_load(uch *indata, struct file *outfp)
{
  	int result;
