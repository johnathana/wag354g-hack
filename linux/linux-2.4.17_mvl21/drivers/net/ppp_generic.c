...
ppp_send_frame(struct ppp *ppp, struct sk_buff *skb)
{
	...
	//junzhao 2005.2.17 for record dstport of ppp frame
	u16 dstport = 0;
	u16 srcport = 0;
    ...
    dstport = (((*pport)& 0x00FF0000) >> 8) | (((*pport) & 0xFF000000) >> 24);
    srcport = (((*pport)& 0x000000FF) << 8) | (((*pport) & 0xFF00FF00) >> 8);
    
    myprintk("\n%x: dstport -- %x(%d) srcport -- %x(%d)\n", *pport, dstport, dstport, srcport, srcport);
    
    if(skb->nfmark == MARK_LAN2WAN)
    {
	myprintk("route through ppp packet!!\n");
    	if(srcport == 520 || dstport == 137 || dstport == 138 || dstport == 139 || dstport == 445)
		skip = 1;
	else
	    	ppp->last_xmit = jiffies;
    }
    
    else if(IP_PROTO(skb) != IPPROTO_ICMP) //don't ignore ping packet!!
    {
  	//junzhao 2005.5.18 for dnsmasq trigger DOD
    	if(strcmp(current->comm, "dnsmasq") || IP_PROTO(skb) != IPPROTO_UDP || dstport != 53)
    	{
		myprintk("self %s send ppp frame, skip!!\n", current->comm);
		skip = 1;
    	}
    }
