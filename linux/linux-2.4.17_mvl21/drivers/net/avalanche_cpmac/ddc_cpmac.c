...
/** \file   ddc_cpmac.c
    ...
    (2) CPMAC_DDC_MIPS_OPTIMIZED should be defined by the makefile if the "MIPS" architectuer optimized 
...
#include "pal.h"            /* PAL SYS/OS services required */
...
#define CPMAC_DDC_MINOR_VERSION         1

const static Char CpmacDDCVersionString[] = "CPMAC DDC version 0.2";
...
Uint32 CpmacDDCDebug = 0;   /* No debug flags by default */
...
LOCAL PAL_Result DDC_cpmacControl(CpmacDDCObj *hDDC, Int cmd, Ptr cmdArg, Ptr param);
LOCAL PAL_Result DDC_cpmacChOpen(CpmacDDCObj *hDDC, CpmacChInfo *chInfo, Ptr chOpenArgs);
LOCAL PAL_Result DDC_cpmacChClose(CpmacDDCObj *hDDC, Int channel, Int direction, Ptr chCloseArgs);

LOCAL PAL_Result cpmacWaitForTeardownComplete(CpmacDDCObj *hDDC, Uint32 channel, DDC_NetChDir direction, Uint32 mode);
LOCAL PAL_Result cpmacEnableChannel(CpmacDDCObj *hDDC, Uint32 channel, Uint32 direction);
LOCAL PAL_Result cpmacDisableChannel(CpmacDDCObj *hDDC, Uint32 channel, DDC_NetChDir direction);
LOCAL PAL_Result cpmacInitTxChannel(CpmacDDCObj *hDDC, CpmacChInfo *chInfo, Ptr chOpenArgs);
LOCAL PAL_Result cpmacInitRxChannel(CpmacDDCObj *hDDC, CpmacChInfo *chInfo, Ptr chOpenArgs);
LOCAL PAL_Result cpmacUnInitTxChannel(CpmacDDCObj *hDDC, Uint32 channel, Ptr chCloseArgs);
LOCAL PAL_Result cpmacUnInitRxChannel(CpmacDDCObj *hDDC, Uint32 channel, Ptr chCloseArgs);

LOCAL void cpmacSetMacAddress(CpmacDDCObj *hDDC, Uint32 channel, String macAddr);
...
LOCAL Int hashAdd(CpmacDDCObj *hDDC, Uint8 *macAddress)
{
    ...
    if (hashValue >= CPMAC_NUM_MULTICAST_BITS)
    {
        LOGERR("\nERROR:DDC: hashAdd:%d: Invalid Hash Value=%d. Should not be greater than %d", 
            hDDC->ddcObj.instId, hashValue, (CPMAC_NUM_MULTICAST_BITS-1));
...
void cpmacSingleMulti(CpmacDDCObj *hDDC, CpmacSingleMultiOper oper, Uint8 *addr)
{
    Int32  status = -1;   
    switch (oper)
    {
...
void cpmacAllMulti(CpmacDDCObj *hDDC, CpmacAllMultiOper oper)
{
    switch (oper)
    {
        ...
        case CPMAC_ALL_MULTI_CLR:
            ...
            break;
        default:
                LOGERR("\nWARN: cpmacAllMulti:%d: Unhandled All multi operation %d", hDDC->ddcObj.instId, oper);
            break;
    }
...
Int cpmacUpdatePhyStatus(CpmacDDCObj *hDDC)
{
    ...
    {
        LOGERR("\nERROR:DDC: cpmacUpdatePhyStatus:%d: Device NOT Open",hDDC->ddcObj.instId);
...
cpmacUpdatePhyStatus_Exit:
    LOGMSG (CPMAC_DEBUG_PORT_UPDATE, 
            ...
            (hDDC->status.PhySpeed)?"100":"10", (hDDC->status.PhyDuplex)?"Full":"Half");
...
LOCAL Int cpmacSetPhyMode(CpmacDDCObj *hDDC)
{
    ...
    if (hDDC->ddcObj.state != DDC_OPENED)
    {
        LOGERR("\nERROR:DDC: cpmacSetPhyMode:%d: Device NOT Open",hDDC->ddcObj.instId);
        return (CPMAC_ERR_DEV_NOT_OPEN);
    }
    ...
    if(setPhyMode & SNWAY_AUTO  )       PhyMode |= NWAY_AUTO;
    if(setPhyMode & SNWAY_FD10  )       PhyMode |= NWAY_FD10;
    if(setPhyMode & SNWAY_FD100 )       PhyMode |= NWAY_FD100;
    if(setPhyMode & SNWAY_HD10  )       PhyMode |= NWAY_HD10;
    if(setPhyMode & SNWAY_HD100 )       PhyMode |= NWAY_HD100;
    if(setPhyMode & SNWAY_LPBK  )       PhyMode |= NWAY_LPBK;
    if(setPhyMode & SNWAY_AUTOMDIX)     PhyMode |= NWAY_AUTOMDIX;

    /* Check for CPMAC Bus frequency for correct speed operation */
    if((setPhyMode & SNWAY_FD10) || (setPhyMode & SNWAY_HD10))
    {
        if (hDDC->initCfg.cpmacBusFrequency <= CPMAC_MIN_FREQUENCY_FOR_10MBPS)
            LOGERR("\nERROR: Bus speedcpmacSetPhyMode:%d: CpmacFreq(%d) is less than required %d freq for 10Mbps support. CANNOT SUPPORTING 10Mbps", 
                hDDC->ddcObj.instId, hDDC->initCfg.cpmacBusFrequency, CPMAC_MIN_FREQUENCY_FOR_10MBPS);
    }
    else if((setPhyMode & SNWAY_FD100) || (setPhyMode & SNWAY_HD100))
    {
        if (hDDC->initCfg.cpmacBusFrequency <= CPMAC_MIN_FREQUENCY_FOR_100MBPS)
            LOGERR("\nERROR: Bus speedcpmacSetPhyMode:%d: CpmacFreq(%d) is less than required %d freq for 100Mbps support. CANNOT SUPPORTING 100Mbps", 
                hDDC->ddcObj.instId, hDDC->initCfg.cpmacBusFrequency, CPMAC_MIN_FREQUENCY_FOR_100MBPS);
    }
    ...
    LOGMSG(CPMAC_DEBUG_PORT_UPDATE, 
            ...
            hDDC->ddcObj.instId, setPhyMode, PhyMode, (PhyMode & NWAY_AUTO), (PhyMode & NWAY_FD10), (PhyMode & NWAY_HD10), 
 ...
 */
LOCAL void cpmacAddType0Addr(CpmacDDCObj *hDDC, Uint32 channel, String macAddress)
{
 ...
 *
 * Thus instead of using below constructs, we use macros to access the registers 

    hDDC->regs->MacAddrLo[channel] =  macAddress[5];
    hDDC->regs->MacAddrMid         =  macAddress[4];
    hDDC->regs->MacAddrHi          = (macAddress[3] << 24) |
                                            (macAddress[2] << 16) |
                                            (macAddress[1] << 8)  |
                                            (macAddress[0]);
...
*/
    ...
    CPMAC_MACADDRHI(hDDC->initCfg.baseAddress) = CSL_FMK(CPMAC_TYPE_0_MACSRCADDR2,(macAddress[3])) |
                                                 CSL_FMK(CPMAC_TYPE_0_MACSRCADDR3,(macAddress[2])) |
 ...
 * It is assumed that this function will get the correct channel number always 
 */
LOCAL void cpmacAddType1Addr(CpmacDDCObj *hDDC, Uint32 channel, String macAddress)
{
...
LOCAL void cpmacAddType2Addr(CpmacDDCObj *hDDC, Uint32 channel, String macAddress, 
                                Int index, Bool valid, Int match)
{
    ...
    /* TODO: Enable Unicast - Is this required ??? */
    cpmacRxUniCast(hDDC, channel, True);
}
...
void cpmacSetRxHwCfg(CpmacDDCObj *hDDC)
{
    ...
    if (hDDC->ddcObj.state != DDC_OPENED)
    {
        LOGERR("\nWARN: cpmacSetRxHwCfg:%d: Function called when device is NOT in open state", hDDC->ddcObj.instId);
        return;
    }
    ...
    LOGMSG(CPMAC_DEBUG_FUNCTION_EXIT, "- cpmacSetRxHwCfg: Rx_MBP_Enable = 0x%08x\n", rxMbpEnable);
}
...
void cpmacSetMacHwCfg(CpmacDDCObj *hDDC)
{
    ...
    if (hDDC->ddcObj.state != DDC_OPENED)
    {
        LOGERR("\nWARN: cpmacSetMacHwCfg:%d: Function called when device is NOT in open state", hDDC->ddcObj.instId);
        return;
    }
...
{ 
 ...
 (DDC_CpmacGetInterruptCause)cpmacGetInterruptCause,  /**< DDC CPMAC function returning interrupt cause */
 (DDC_CpmacPktTxCompletionProcess)cpmacTxPktCompletionProcess,       /**< DDC CPMAC Tx packet processing function */
 (DDC_CpmacPktRxProcess)cpmacRxPktProcess       /**< DDC CPMAC Rx packet processing function */
};
 ...
 */
PAL_Result DDC_cpmacCreateInstance(Uint32 instId, DDA_Handle hDDA, CpmacDDACbIf *hDDACbIf,
                                   DDC_Handle **hDDC, CpmacDDCIf **hDDCIf, Ptr param)
{
...
PAL_Result DDC_cpmacDeleteInstance (CpmacDDCObj *hDDC, Ptr param)
{
    ...
    {
        LOGERR("\nERROR:DDC: DDC_cpmacDeleteInstance:%d: Instance NOT created / Already deleted", instId);
...
LOCAL PAL_Result DDC_cpmacInit(CpmacDDCObj *hDDC, CpmacInitConfig *initCfg)
{
    LOGMSG(CPMAC_DEBUG_FUNCTION_ENTRY, "\n+ DDC_cpmacInit %d", hDDC->ddcObj.instId);
    ...
    {
        LOGERR("\nERROR:DDC: DDC_cpmacInit:%d: Invalid number of TX/RX channels", hDDC->ddcObj.instId);
        return (CPMAC_INVALID_PARAM);
    }
...
LOCAL PAL_Result DDC_cpmacDeInit (CpmacDDCObj *hDDC, Ptr param)
{
    ...
    LOGMSG(CPMAC_DEBUG_FUNCTION_EXIT, "\n- DDC_cpmacDeInit:%d:", hDDC->ddcObj.instId);
    return (CPMAC_SUCCESS);
}
...
LOCAL PAL_Result DDC_cpmacOpen(CpmacDDCObj *hDDC, Ptr param)
{
    ...
    LOGMSG(CPMAC_DEBUG_FUNCTION_ENTRY, "\n+ DDC_cpmacOpen:%d:", hDDC->ddcObj.instId);

    if (hDDC->ddcObj.state == DDC_OPENED)
    {
        LOGERR("\nERROR:DDC: DDC_cpmacOpen:%d: Device already open", hDDC->ddcObj.instId);
        return (CPMAC_ERR_DEV_ALREADY_OPEN);
    }

    /* Bring CPMAC out of reset - for clean implementation, reset and then unreset the module */
    PAL_sysResetCtrl(hDDC->initCfg.resetLine, IN_RESET);
    PAL_sysResetCtrl(hDDC->initCfg.resetLine,OUT_OF_RESET);
    PAL_osWaitMsecs(1);
    ...
    cpswHalCommonMiiMdioGetVer(initCfg->mdioBaseAddress, &miiModId,  &miiRevMaj,  &miiRevMin);
    ...
    retCode = PAL_osMemAlloc(0, cpswHalCommonMiiMdioGetPhyDevSize(), 0, (Ptr *)&hDDC->PhyDev);  
    if (retCode != PAL_SOK)
    {
        LOGERR("\nERROR:DDC: DDC_cpmacOpen:%d: Failed to allocate memory for Phy Device", hDDC->ddcObj.instId);
        return (retCode);
    }
    ...
    cpswHalCommonMiiMdioInit (hDDC->PhyDev, 
                                ...
                                initCfg->PhyMask, 
                                initCfg->MLinkMask, 
                                0,                      /* ResetBase param not required for this driver */
                                ...
                                (CpmacDDCDebug & CPMAC_DEBUG_MII),  /* previously HalDev->debug */
    ...
    hDDC->ddaIf->ddaNetIf.ddaFuncTable.ddaControlCb(hDDC->ddcObj.hDDA, CPMAC_DDA_IOCTL_TIMER_START, (Ptr) initCfg->MdioTickMSec, NULL);
    ...
    for (channel = 0; channel < CPMAC_MAX_TX_CHANNELS; channel++)
    {
        if (hDDC->txCppi[channel] != NULL)
        {
            ...
            {
                LOGERR("\nERROR:DDC: DDC_cpmacOpen:%d: Error enabling TX channel %d", hDDC->ddcObj.instId, channel);
    ...
    for (channel = 0; channel < CPMAC_MAX_RX_CHANNELS; channel++)
    {
        if (hDDC->rxCppi[channel] != NULL)
        {
            ...
            if (retVal != CPMAC_SUCCESS)
            {
                LOGERR("\nERROR:DDC: DDC_cpmacOpen:%d: Error enabling RX channel %d", hDDC->ddcObj.instId, channel);
                /* TODECIDE: Should we return from here or continue enabling other channels */ 
                return (retVal); 
            }
        }
    ...
    hDDC->ddaIf->ddaNetIf.ddaFuncTable.ddaControlCb(hDDC->ddcObj.hDDA, CPMAC_DDA_IOCTL_MIB64_CNT_TIMER_START,
		                                    (Ptr) initCfg->Mib64CntMsec, NULL);
...
LOCAL PAL_Result DDC_cpmacClose(CpmacDDCObj *hDDC, Ptr param)
{
    ...
    LOGMSG(CPMAC_DEBUG_FUNCTION_ENTRY, "\n+ DDC_cpmacClose:%d:", hDDC->ddcObj.instId);

    if (hDDC->ddcObj.state == DDC_CLOSED)
    {
        LOGERR("\nERROR:DDC: DDC_cpmacClose:%d: Device already closed", hDDC->ddcObj.instId);
    ...
    hDDC->ddaIf->ddaNetIf.ddaFuncTable.ddaControlCb(hDDC->ddcObj.hDDA, CPMAC_DDA_IOCTL_MIB64_CNT_TIMER_STOP,
		                                    NULL, NULL);
    ...
    for (channel = 0; channel < CPMAC_MAX_TX_CHANNELS; channel++)
    {
        if (hDDC->txCppi[channel] != NULL)
        {
            ...
            if (retVal != CPMAC_SUCCESS)
            {
                LOGERR("\nERROR:DDC: DDC_cpmacOpen:%d: Error closing TX channel %d", hDDC->ddcObj.instId, channel);
    ...
    for (channel = 0; channel < CPMAC_MAX_RX_CHANNELS; channel++)
    {
        if (hDDC->rxCppi[channel] != NULL)
        {
            ...
            if (retVal != CPMAC_SUCCESS)
            {
                LOGERR("\nERROR:DDC: DDC_cpmacOpen:%d: Error closing RX channel %d", hDDC->ddcObj.instId, channel);
...
LOCAL PAL_Result DDC_cpmacControl(CpmacDDCObj *hDDC, Int cmd, Ptr cmdArg, Ptr param)
{
    switch (cmd)
    {
        case CPMAC_DDC_IOCTL_GET_SWVER:
                ...
                *((Uint32 *)cmdArg) = (Uint32) ((CPMAC_DDC_MAJOR_VERSION << 16) | CPMAC_DDC_MINOR_VERSION);
        ...
        case CPMAC_DDC_IOCTL_GET_STATUS:
                ...
                if (hDDC->ddcObj.state != DDC_OPENED)
                {
                    LOGERR("\nWARN: DDC_cpmacControl:%d: DDC_CPMAC_IOCTL_GET_STATUS Ioctl called when device is NOT in open state", hDDC->ddcObj.instId);
                    return (CPMAC_ERR_DEV_NOT_OPEN);
                }
        ...
        case CPMAC_DDC_IOCTL_READ_PHY_REG:
                ...
                if (hDDC->ddcObj.state != DDC_OPENED)
                {
                    LOGERR("\nWARN: DDC_cpmacControl:%d: CPMAC_DDC_IOCTL_READ_PHY_REG Ioctl called when device is NOT in open state", hDDC->ddcObj.instId);
                    return (CPMAC_ERR_DEV_NOT_OPEN);
                }
                {
                    ...
                    phyParams->data = _cpswHalCommonMiiMdioUserAccessRead(hDDC->PhyDev, phyParams->regAddr, phyParams->phyNum);
                }
            break;
        ...
        case CPMAC_DDC_IOCTL_WRITE_PHY_REG:
                ...
                if (hDDC->ddcObj.state != DDC_OPENED)
                {
                    LOGERR("\nWARN: DDC_cpmacControl:%d: CPMAC_DDC_IOCTL_WRITE_PHY_REG Ioctl called when device is NOT in open state", hDDC->ddcObj.instId);
                    return (CPMAC_ERR_DEV_NOT_OPEN);
                }
                {
                    CpmacPhyParams *phyParams = (CpmacPhyParams *) cmdArg;
                    ...
                    /* \warning: Write to the phy registers - Note that this code loops 
                       on a completion bit in the phy so there are chances of hanging" */
                    _cpswHalCommonMiiMdioUserAccessWrite(hDDC->PhyDev, phyParams->regAddr, phyParams->phyNum, phyParams->data);
                }
            break;
        ...
        case CPMAC_DDC_IOCTL_GET_STATISTICS:
                ...
                if (hDDC->ddcObj.state != DDC_OPENED)
                {
                    LOGERR("\nWARN: DDC_cpmacControl:%d: CPMAC_DDC_IOCTL_GET_STATISTICS Ioctl called when device is NOT in open state", hDDC->ddcObj.instId);
                    return (CPMAC_ERR_DEV_NOT_OPEN);
                }
                {   
                    Uint32 cnt;
        ...
        case CPMAC_DDC_IOCTL_CLR_STATISTICS:
                ...
                if (hDDC->ddcObj.state != DDC_OPENED)
                {
                    LOGERR("\nWARN: DDC_cpmacControl:%d: CPMAC_DDC_IOCTL_CLR_STATISTICS Ioctl called when device is NOT in open state", hDDC->ddcObj.instId);
                    return (CPMAC_ERR_DEV_NOT_OPEN);
                }
                {
                    Uint32 cnt;
        ...
        case CPMAC_DDC_IOCTL_MULTICAST_ADDR:
                ...
                if (hDDC->ddcObj.state != DDC_OPENED)
                {
                    LOGERR("\nWARN: DDC_cpmacControl:%d: CPMAC_DDC_IOCTL_MULTICAST_ADDR Ioctl called when device is NOT in open state", hDDC->ddcObj.instId);
                    return (CPMAC_ERR_DEV_NOT_OPEN);
                }
                {
        ...
        case CPMAC_DDC_IOCTL_TYPE2_3_FILTERING:
            {
                ...
                if (hDDC->ddcObj.state != DDC_OPENED)
                {
                    LOGERR("\nWARN: DDC_cpmacControl:%d: CPMAC_DDC_IOCTL_TYPE2_3_FILTERING Ioctl called when device is NOT in open state", hDDC->ddcObj.instId);
                    return (CPMAC_ERR_DEV_NOT_OPEN);
                }
		...
		case CPMAC_DDC_IOCTL_IF_COUNTERS:			
			if (hDDC->ddcObj.state != DDC_OPENED)
			{
            	LOGERR("\nWARN: DDC_cpmacControl:%d: CPMAC_DDC_IOCTL_IF_COUNTERS Ioctl called when device 
								is NOT in open state", hDDC->ddcObj.instId);
				return (CPMAC_ERR_DEV_NOT_OPEN);
			}		
		...
		case CPMAC_DDC_IOCTL_ETHER_COUNTERS:     		
			if (hDDC->ddcObj.state != DDC_OPENED)
			{
            	LOGERR("\nWARN: DDC_cpmacControl:%d: CPMAC_DDC_IOCTL_ETHER_COUNTERS Ioctl called when device 
								is NOT in open state", hDDC->ddcObj.instId);
				return (CPMAC_ERR_DEV_NOT_OPEN);
			}		
		...
		case CPMAC_DDC_IOCTL_IF_PARAMS_UPDT: 	    
			if (hDDC->ddcObj.state != DDC_OPENED)
			{
            	LOGERR("\nWARN: DDC_cpmacControl:%d: CPMAC_DDC_IOCTL_IF_PARAMS_UPDT Ioctl called when device 
								is NOT in open state", hDDC->ddcObj.instId);
				return (CPMAC_ERR_DEV_NOT_OPEN);
			}
			cpmacDDCIfcntUpdt(hDDC);
        ...
        default:
                LOGERR("\nWARN: DDC_cpmacControl:%d: Unhandled ioctl code %d", hDDC->ddcObj.instId, cmd);
            break;
    }   
...
LOCAL PAL_Result DDC_cpmacChOpen(CpmacDDCObj *hDDC, CpmacChInfo *chInfo, Ptr chOpenArgs)
{
    ...
    LOGMSG(CPMAC_DEBUG_FUNCTION_ENTRY, "\n+ DDC_cpmacChOpen:%d: ChannelNo=%d, Dir=%s", 
        hDDC->ddcObj.instId, chInfo->chNum, ((chInfo->chDir == DDC_NET_CH_DIR_TX) ? "TX" : "RX") );
    ...
    if (chInfo->chState != DDC_NET_CH_UNINITIALIZED)
    {
        LOGERR("\nERROR:DDC: DDC_cpmacChOpen:%d: %s channel %d  should be in DDC_NET_CH_UNINITIALIZED state", 
            hDDC->ddcObj.instId, ((chInfo->chDir == DDC_NET_CH_DIR_TX) ? "TX" : "RX"), chInfo->chNum);
        return (CPMAC_INVALID_PARAM);
    }
...
LOCAL PAL_Result DDC_cpmacChClose(CpmacDDCObj *hDDC, Int channel, Int direction, Ptr chCloseArgs)
{
    ...
    LOGMSG(CPMAC_DEBUG_FUNCTION_ENTRY, "\n+ DDC_cpmacChClose:%d: ChannelNo=%d, Dir=%s", 
        hDDC->ddcObj.instId, channel, ((direction == DDC_NET_CH_DIR_TX) ? "TX" : "RX") );
...
LOCAL PAL_Result cpmacInitTxChannel(CpmacDDCObj *hDDC, CpmacChInfo *chInfo, Ptr chOpenArgs)
{
    ...
    retCode = PAL_osMemAlloc(0, (chInfo->serviceMax * sizeof(Uint32)), 4, (Ptr *)&txCppi->txComplete);  
    if (retCode != PAL_SOK)
    {
    ...
    /* Set memory to 0 */
    PAL_osMemSet(txCppi->txComplete, 0, (chInfo->serviceMax * sizeof(Uint32)));
#endif
...
LOCAL PAL_Result cpmacUnInitTxChannel(CpmacDDCObj *hDDC, Uint32 channel, Ptr chCloseArgs)
{
    ...
    retCode = PAL_osMemFree(0, txCppi->txComplete, (txCppi->chInfo.serviceMax * sizeof(Uint32)));
    if (retCode != PAL_SOK)
    {
...
LOCAL PAL_Result cpmacInitRxChannel(CpmacDDCObj *hDDC, CpmacChInfo *chInfo, Ptr chOpenArgs)
{
    ...
    retCode = PAL_osMemAlloc(0, (chInfo->serviceMax * sizeof(DDC_NetPktObj)), 4, (Ptr *)&rxCppi->pktQueue);  
    if (retCode != PAL_SOK)
    {
    ...
    PAL_osMemSet(rxCppi->pktQueue, 0, (chInfo->serviceMax * sizeof(DDC_NetPktObj)));

    /* Allocate memory for buffer queue on a 4 byte boundry and set to 0 */
    retCode = PAL_osMemAlloc(0, (chInfo->serviceMax * sizeof(DDC_NetBufObj) * CPMAC_MAX_RX_FRAGMENTS), 4, (Ptr *)&rxCppi->bufQueue);  
    if (retCode != PAL_SOK)
    {
        LOGERR("\nERROR:DDC: cpmacInitRxChannel:%d: Failed to allocate memory for RX buffer queue %d", 
            hDDC->ddcObj.instId, chInfo->chNum);
        /* Free pkt queue */
        PAL_osMemFree(0, rxCppi, (chInfo->serviceMax * sizeof(DDC_NetPktObj)));
        /* Free tx cppi channel memory */
        PAL_osMemFree(0, rxCppi, sizeof(CpmacRxCppiCh));
        return (retCode);
    }
    PAL_osMemSet(rxCppi->bufQueue, 0, (chInfo->serviceMax * sizeof(DDC_NetBufObj) * CPMAC_MAX_RX_FRAGMENTS));
    ...
    for (cnt = 0; cnt < chInfo->numBD; cnt++)
    {
        ...
        currBD->dataPtr = (Ptr) (hDDC->ddaIf->ddaNetIf.ddaNetAllocRxBufCb(hDDC->ddcObj.hDDA, chInfo->bufSize, (DDC_NetDataToken *)&currBD->bufToken, (Ptr) currBD));
        if (currBD->dataPtr == NULL)
        {
            LOGERR("\nERROR:DDC: cpmacInitRxChannel: Error in RX Buffer allocation for channel %d", chInfo->chNum);
...
LOCAL PAL_Result cpmacUnInitRxChannel(CpmacDDCObj *hDDC, Uint32 channel, Ptr chCloseArgs)
{
    ...
    while (currBD)
    {
        if (hDDC->ddaIf->ddaNetIf.ddaNetFreeRxBufCb(hDDC->ddcObj.hDDA, currBD->dataPtr, (DDC_NetDataToken) currBD->bufToken, NULL) != CPMAC_SUCCESS)
        {
            LOGERR("\nERROR:DDC: cpmacUnInitRxChannel:%d: Failed to free RX buffer Ch %d", hDDC->ddcObj.instId, channel );
        }
        currBD = currBD->next;
    }
...

{
        ...
        if (hDDC->RxAddrType == RX_ADDR_TYPE0)
            ...
            LOGERR("\nWARN: cpmacEnableChannel:%d: Wrong Rx Addressing Type - (Type2) detected in hardware", hDDC->ddcObj.instId);
}
...

{
    ...
    if (direction == DDC_NET_CH_DIR_TX)
    {
        ...
        {
            LOGERR("\nERROR:DDC: cpmacDisableChannel:%d: Failed to teardown TX channel %d", hDDC->ddcObj.instId, channel );
    ...
    else if (direction == DDC_NET_CH_DIR_RX)
    {
        ...
        if (cpmacWaitForTeardownComplete(hDDC, channel, direction, True) != CPMAC_SUCCESS)
        {
            LOGERR("\nERROR:DDC: cpmacDisableChannel:%d: Failed to teardown RX channel %d", hDDC->ddcObj.instId, channel );
        }
 ...
 */
LOCAL PAL_Result cpmacWaitForTeardownComplete(CpmacDDCObj *hDDC, Uint32 channel, DDC_NetChDir direction, Bool blocking)
{
    if (direction == DDC_NET_CH_DIR_TX)
    {
        CpmacTxBD       *currBD;
        ...
        while ( (hDDC->regs->Tx_CP[channel] & CPMAC_TEARDOWN_VALUE) != CPMAC_TEARDOWN_VALUE) 
        {
            ...
            /* TODO: A task delay can be called here to pend rather than occupying CPU cycles - anyway
               it has been found that the teardown takes very few cpu cycles and does not affect functionality */
        }
    ...
    else if (direction == DDC_NET_CH_DIR_RX)
    {
        ...
        while ( (hDDC->regs->Rx_CP[channel] & CPMAC_TEARDOWN_VALUE) != CPMAC_TEARDOWN_VALUE) 
        {
            ...
            /* TODO: A task delay can be called here to pend rather than occupying CPU cycles */
        }
        /* Write to the completion pointer */
...
LOCAL void cpmacDDCIfcntClear(CpmacDDCObj *hDDC)
{
     PAL_osMemSet((char *)&hDDC->Mib2IfHCCounter, 0, sizeof(hDDC->Mib2IfHCCounter));
}
...
LOCAL void cpmacDDCIfcntUpdt(CpmacDDCObj *hDDC)
{
	 ...
	 else
	 {
		 ...
		 hDDC->Mib2IfHCCounter.inUnicastPktsHC +=0xffffffff - (hDDC->Mib2IfHCCounter.inUnicastPkts -
	...
	{
		hDDC->Mib2IfHCCounter.outUnicastPktsHC +=((stats.ifOutGoodFrames - 
               								    (stats.ifOutBroadcasts + stats.ifOutMulticasts))
						                         - hDDC->Mib2IfHCCounter.outUnicastPkts);	
	}
	else
	{
		hDDC->Mib2IfHCCounter.outUnicastPktsHC +=0xffffffff - (hDDC->Mib2IfHCCounter.outUnicastPkts -
    ...
    hDDC->Mib2IfHCCounter.Mib2IfCounter.inBytesLow = (unsigned long)hDDC->Mib2IfHCCounter.inBytesHC;
    /* high 32-bit of total octets received from media */
	hDDC->Mib2IfHCCounter.Mib2IfCounter.inBytesHigh = ( hDDC->Mib2IfHCCounter.inBytesHC >> 32);


	/* low 32-bit of total octets received from media */
    hDDC->Mib2IfHCCounter.Mib2IfCounter.inUnicastPktsLow = (unsigned long)hDDC->Mib2IfHCCounter.inUnicastPktsHC;
    /* high 32-bit of total octets received from media */
	hDDC->Mib2IfHCCounter.Mib2IfCounter.inUnicastPktsHigh= (hDDC->Mib2IfHCCounter.inUnicastPktsHC >> 32);


	/* low 32-bit of total octets received from media */
    hDDC->Mib2IfHCCounter.Mib2IfCounter.inMulticastPktsLow = (unsigned long)hDDC->Mib2IfHCCounter.inMulticastPktsHC;
    /* high 32-bit of total octets received from media */
	hDDC->Mib2IfHCCounter.Mib2IfCounter.inMulticastPktsHigh= hDDC->Mib2IfHCCounter.inMulticastPktsHC >> 32;
	
	/* low 32-bit of total octets received from media */
    hDDC->Mib2IfHCCounter.Mib2IfCounter.inBroadcastPktsLow = (unsigned long)hDDC->Mib2IfHCCounter.inBroadcastPktsHC;
    /* high 32-bit of total octets received from media */
	hDDC->Mib2IfHCCounter.Mib2IfCounter.inBroadcastPktsHigh= hDDC->Mib2IfHCCounter.inBroadcastPktsHC >> 32;
   ...
   /* packets discarded due to format errors */
	...
	/* low 32-bit of total octets received from media */
    hDDC->Mib2IfHCCounter.Mib2IfCounter.outBytesLow = (unsigned long)hDDC->Mib2IfHCCounter.outBytesHC;
    /* high 32-bit of total octets received from media */
	hDDC->Mib2IfHCCounter.Mib2IfCounter.outBytesHigh = hDDC->Mib2IfHCCounter.outBytesHC >> 32;


	/* low 32-bit of total octets received from media */
    hDDC->Mib2IfHCCounter.Mib2IfCounter.outUnicastPktsLow = (unsigned long)hDDC->Mib2IfHCCounter.outUnicastPktsHC;
    /* high 32-bit of total octets received from media */
	hDDC->Mib2IfHCCounter.Mib2IfCounter.outUnicastPktsHigh = hDDC->Mib2IfHCCounter.outUnicastPktsHC >> 32;

	/* low 32-bit of total octets received from media */
    hDDC->Mib2IfHCCounter.Mib2IfCounter.outMulticastPktsLow = (unsigned long)hDDC->Mib2IfHCCounter.outMulticastPktsHC;
    /* high 32-bit of total octets received from media */
	hDDC->Mib2IfHCCounter.Mib2IfCounter.outMulticastPktsHigh= hDDC->Mib2IfHCCounter.outMulticastPktsHC >> 32;
	
	/* low 32-bit of total octets received from media */
    hDDC->Mib2IfHCCounter.Mib2IfCounter.outBroadcastPktsLow = (unsigned long)hDDC->Mib2IfHCCounter.outBroadcastPktsHC;
    /* high 32-bit of total octets received from media */
	hDDC->Mib2IfHCCounter.Mib2IfCounter.outBroadcastPktsHigh = hDDC->Mib2IfHCCounter.outBroadcastPktsHC >> 32;
  ...
  /* packets discarded due to resource limit */
    hDDC->Mib2IfHCCounter.Mib2IfCounter.outDiscardPkts = stats.ifOutUnderrun+hDDC->Mib2IfHCCounter.Mib2IfCounter.outErrorPkts;    	   
	return;		
}	
...
void cpmacDDCStats(Uint32 instId)
{
    ...
    {
        printf("\ncpmacDumpRxStat: Invalid instance id %d. Currently only %d instances available", instId, CpmacDDCNumInst);
        return;
    }
    ...
    if (hDDC == NULL)
    {
        printf("\ncpmacDumpRxStat: Invalid DDC handle for instance id %d.", instId);
        return;
    }
    ...
    {
        printf("\n");
        printf("\nTX Proc Count                 = %u", txCppi->procCount);
        printf("\nTX Queue Reinit               = %u", txCppi->queueReinit);
        printf("\nTX End of Queue Add in Send   = %u", txCppi->endOfQueueAdd);
        printf("\nTX Active Queue Count         = %u", txCppi->activeQueueCount);
        printf("\nTX Out of BD's                = %u", txCppi->outOfTxBD);
        printf("\nTX Int when no active pkts    = %u", txCppi->noActivePkts);
        printf("\nPkts with multifragments      = %u", txCppi->numMultiFragPkts);
    }
    ...
    {
        printf("\n");
        printf("\nRX Proc Count                 = %u", rxCppi->procCount);
        printf("\nRX Processed BD's             = %u", rxCppi->processedBD);
        printf("\nRX Recycled BD's              = %u", rxCppi->recycledBD);
        printf("\nRX Out of BD's                = %u", rxCppi->outOfRxBD);
        printf("\nRX Out of Buffers             = %u", rxCppi->outOfRxBuffers);
        printf("\nRX Queue reinit               = %u", rxCppi->queueReinit);
        printf("\nRX End of queue Add           = %u", rxCppi->endOfQueueAdd);
        printf("\nRX End of queue               = %u", rxCppi->endOfQueue);
        printf("\nRX Misqueued packets          = %u", rxCppi->misQueuedPackets);
        printf("\nPkts with multifragments      = %u", rxCppi->numMultiFragPkts);
    }
    printf("\n");
